describe("Ext.container.Container", function () {
  ...
  describe("onRender sequence", function () {
    ...
    it("should call onRender passing parentNode and position", function () {
      ...
      expect(Ext.Component.prototype.onRender.calls[0].args[0]).toEqual(
        Ext.getBody(),
      );
      expect(Ext.Component.prototype.onRender.calls[0].args[1]).toBeUndefined();
      expect(Ext.Component.prototype.onRender.calls[1].args[0]).toEqual(
        ct.layout.innerCt,
      );
  ...
  describe("queryBy", function () {
    ...
    it("should only exclude items if the return value is false", function () {
      ...
      expect(ct.queryBy(function (c) {})).toEqual([c1, c2, c3]);
    });
  ...
  describe("enable/disable", function () {
    var disableFn, a, b, c, a1, a2, a3, b1, b2, b3;
  ...
  describe("nextChild", function () {
    ...
    it("should return the next child using a selector", function () {
      expect(container.nextChild(age, "field[name=bio]")).toBe(
        container.getComponent(4),
      );
    });
  });
  ...
  describe("prevChild", function () {
    ...
    it("should return the previous child using a selector", function () {
      expect(container.prevChild(age, "field[name=name]")).toBe(
      ...
      var container = new Ext.panel.Panel({
          ...
          items: [
            ...
            },
            {
              xtype: "gridpanel",
              columns: [
                { text: "Col1", dataIndex: "field1", variableRowHeight: true },
              ],
              store: {
                fields: ["field1"],
                data: [{ field1: "value1" }, { field1: "value2" }],
  ...
  describe("hierarchy state", function () {
    ...
    function createHierarchySuite(mode) {
      ...
      describe(
        "hierarchical " + (mode === "hidden" ? "hiding" : "collapsing"),
        function () {
      ...
      function createShowExpandSuite(initialRenderHiddenOrCollapsed) {
        describe(
          "hierarchical " +
            (mode === "hidden" ? "showing" : "expanding") +
            " (parent " +
            (initialRenderHiddenOrCollapsed
              ? "initially rendered " + mode
              : mode + " after inital render") +
            ")",
          function () {
            ...
            it(
              "should remove " + mode + " from the hierarchy state",
              function () {
                a[showOrExpand]();
                ...
                expect(mode in fb.getInherited()).toBe(
      ...
      describe(
        "moving from " + oppositeMode + " parent to " + mode + " parent",
        function () {
  ...
  describe("cascade", function () {
    ...
    describe("call order", function () {
      ...
      it("should call depth first", function () {
        ...
        ct.cascade(spy);
        expectCalls([ct, a, a1, a2, a3, b, b1, c, c1, c2, d, d1, d2, d3]);
      });
    });
