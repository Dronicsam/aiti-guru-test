describe("Ext.grid.feature.Grouping", function () {
  ...
  describe("mouse interaction", function () {
    ...
    (Ext.isIE ? xit : it)("should respond to mouse events", function () {
      ...
      runs(function () {
        ...
        var firstInGroup1 = lockedGrid.store.data.findBy(function (r) {
          return (
            r.data.description === 'Add required rendering "hooks" to GridView'
          );
        });

        var firstRowInGroup1 =
          lockedGrid.normalGrid.view.getNode(firstInGroup1);
        var dataRowOfFirstRowInGroup1 =
          lockedGrid.normalGrid.view.getRow(firstInGroup1);
      ...
      runs(function () {
        ...
        expect(
          (group0SummaryRow.innerText || group0SummaryRow.textContent)
            .replace(/\n/g, "")
            .replace(/\r/g, ""),
    ...
    it("should collapse all other groups when CRTL/click on group header", function () {
      ...
      runs(function () {
        var grouping =
            lockedGrid.lockedGrid.view.findFeature("groupingsummary"),
        ...
        expect(groupStore.getCount()).toEqual(
          lockedGridStore.getCount() - firstGroup.getRange().length + 1,
        );

        // CTRL/click on the first group's header
        grouping.onGroupClick(
        ...
        expect(groupStore.getCount()).toEqual(
          Ext.Object.getKeys(grouping.getCache().map).length -
            1 +
            firstGroup.getRange().length,
        );
      });
    });
  });
    ...
    describe("init", function () {
      describe("startCollapsed", function () {
        function startCollapsed(bool) {
          it(
            "should honor the `startCollapsed` config when " + bool,
            function () {
    ...
    describe("collapse state after grid store operations", function () {
      describe("initial collapse state", function () {
        function collapsed(collapseState, filterValue) {
          it(
            "should retain its collapsed state of " +
              filterValue +
              " after the grid store is filtered and cleared",
            function () {
              makeUI(null, {
  ...
  describe("the metaGroupCache", function () {
    ...
    describe("locked grids", function () {
      ...
      it("should share this object with any locking partner", function () {
        expect(lockedGroupingFeature.lockingPartner.metaGroupCache).toBe(
          lockedGroupingFeature.metaGroupCache,
        );
        expect(normalGroupingFeature.lockingPartner.metaGroupCache).toBe(
          normalGroupingFeature.metaGroupCache,
        );
      });
  ...
  function runGroupers(buffered) {
    describe(
      "groupers and " + (buffered ? "buffered" : "data") + " store",
      function () {
        ...
        describe("defined groupers", function () {
                  ...
                  describe("metaGroupCache", function () {
                    ...
                    describe("tpl values", function () {
                      ...
                      it('should have a "groupValue" value determined by looking up the groupField on the record', function () {
                        ...
                        expect(cache.groupValue).toBe(
                          getRec(1).get(cache.groupField),
                        );
                      });
                    });
                  });
          ...
          describe("grouping via the UI", function () {
            ...
            describe('the "Group by this field" menu item', function () {
              function doTest() {
                // Sanity.
                expect(groupingFeature.metaGroupCache.groupField).toBe(
                  "cuisine",
                );
                expect(groupingFeature.metaGroupCache.groupValue).toBe(
                  data[0].cuisine,
                );
    ...
    describe("operating on the last field in a group", function () {
      function createGrid(groupField) {
        makeGrid({
          ...
          data: [{ name: "Beardog's", cuisine: true }],
          filters: [
            {
              property: "cuisine",
              value: true,
