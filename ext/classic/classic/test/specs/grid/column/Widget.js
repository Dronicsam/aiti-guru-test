describe("Ext.grid.column.Widget", function () {
  ...
  function getPadding() {
    ...
    return (
      parseInt(cell.getStyle("padding-left"), 10) +
  ...
  describe("widget refocus on row delete", function () {
    ...
    webkitIt(
      "should not cause an error when deleting the focused row using an actionable widget",
      function () {
        createGrid([
          {
            itemId: "ct",
  ...
  function createBufferedSuite(withBuffered) {
    describe(
      withBuffered ? "with buffered rendering" : "without buffered rendering",
      function () {
