describe("Ext.form.field.ComboBox", function () {
  ...
  describe("onExpand", function () {
    ...
    it("should set the BoundList's selection to match the current value", function () {
      ...
      expect(component.picker.selModel.getSelection()[0].get("val")).toEqual(
        component.value,
      );
    });
    it("should initialize a BoundListKeyNav on the BoundList", function () {
      ...
      expect(
        component.getPicker().getNavigationModel() instanceof
  ...
  describe("setting value", function () {
    ...
    describe("setValue method", function () {
      ...
      it("should update the expanded dropdown's selection - single select", function () {
        ...
        runs(function () {
          component.setValue("value 2");
          expect(component.picker.getSelectionModel().getSelection()).toEqual([
            store.getAt(1),
          ]);
        });
      });
  ...
  describe("getting value", function () {
    ...
    it("should return the valueField for an item selected from the list", function () {
      ...
      runs(function () {
        component.picker
          .getSelectionModel()
          .select([store.findRecord("text", "text 2")]);
  ...
  describe("growToLongestValue", function () {
    var bodyEl,
      ...
      longestText =
        "this text is much, much, much, much, much, much, much, much, much much, much, much, much, much, much, much, much too long";
  ...
  describe("doQuery method", function () {
    ...
    describe("remote queryMode", function () {
      it("should call the store's load method", function () {
        ...
        expect(component.store.load.calls[0].args[0].params.query).toEqual(
          "foobar",
        );
      });
      it("should pass the query string using the 'queryParam' as the parameter name", function () {
        ...
        expect(component.store.load.callCount).toEqual(1);
        expect(
          component.store.load.calls[0].args[0].params.customparam,
        ).toEqual("foobar");
      });
    });
  ...
  describe("doAutoSelect method", function () {
    it("should highlight the selected item", function () {
      ...
      spyOn(
        component.picker.getNavigationModel(),
        "setPosition",
      ).andCallThrough();
      ...
      expect(
        component.picker.getNavigationModel().setPosition,
      ).toHaveBeenCalled();
    ...
    it("should scroll the selected item into view", function () {
      ...
      expect(
        component.picker.getScrollable().scrollIntoView,
      ).toHaveBeenCalled();
    });
  });
  ...
  describe("keyboard input", function () {
    ...
    it("should initiate a query after the queryDelay", function () {
      ...
      runs(function () {
        expect(component.doQuery.mostRecentCall.args).toEqual([
          "foob",
          false,
          true,
        ]);
    ...
    itNotIE("should select the value upon tab", function () {
      ...
      runs(function () {
        ...
        expect(component.getRawValue()).toBe(
          selModel.getSelection()[0].get(component.displayField),
        );
      });
    });
  ...
  describe("forceSelection", function () {
    ...
    describe("not multi", function () {
      ...
      describe("with remote loading", function () {
        ...
        (Ext.isGecko ? xit : it)(
          "should clear an unmatched value when the store loads",
          function () {
            store.destroy();
            store = new Ext.data.Store({
              model: CBTestModel,
    ...
    describe("with remote loading", function () {
      ...
      itNotIE(
        "should clear an unmatched value when the store loads, second version!",
        function () {
          store.destroy();
          store = new Ext.data.Store({
            model: CBTestModel,
    ...
    beforeEach(function () {
      combo = Ext.create("Ext.form.field.ComboBox", {
        ...
        beforeQuery: function () {
          var result = Ext.form.field.ComboBox.prototype.beforeQuery.apply(
            this,
            arguments,
          );
  ...
  describe("displayTpl", function () {
    describe("should create default", function () {
      ...
      it("displayTpl html match", function () {
        expect(component.displayTpl.html).toBe(
          '<tpl for=".">{[typeof values === "string" ? values : values["text"]]}<tpl if="xindex < xcount">, </tpl></tpl>',
        );
      });
    });
    ...
    describe("should create from string", function () {
      beforeEach(function () {
        makeComponent({
          displayTpl:
            '<tpl for=".">{[typeof values === "string" ? values : values["foo"]]}</tpl>',
        });
      });
  ...
  describe("setting value with different store states", function () {
    ...
    describe("setting a value with a remote store", function () {
      var fakeData, remoteStore, ComboModel;
          ...
          createPicker: function () {
            panel = new Ext.grid.Panel({
              ...
              store: new Ext.data.ArrayStore({
                storeId: "baz",
                fields: [{ name: "company" }, { name: "price", type: "float" }],
                data: [
                  ["3m Co", 71.72],
                  ["Alcoa Inc", 29.01],
                  ["Boeing Co.", 75.43],
  ...
  describe("complex binding", function () {
    ...
    it("should publish a selection when store provided by a bind is NOT autoloaded, value is configured and displayField === valueField", function () {
      ...
      waitsFor(function () {
        ...
        return (
          targetComp.el.dom.innerHTML === "Red" &&
          viewModel.get("color") === store.byText.get("Red")
        );
      });
    });
