describe("Ext.button.Segmented", function () {
  ...
  describe("item classes", function () {
    ...
    it("should have the correct classes when items are shown or hidden", function () {
      ...
      items.getAt(3).show();
      expectClasses([items.getAt(1), items.getAt(3)]);

      items.getAt(0).show();
      expectClasses([items.getAt(0), items.getAt(1), items.getAt(3)]);
      ...
      items.getAt(1).hide();
      expectClasses([items.getAt(0), items.getAt(2), items.getAt(3)]);

      items.getAt(3).hide();
      expectClasses([items.getAt(0), items.getAt(2)]);

      items.getAt(0).hide();
      expectClasses([items.getAt(2)]);
    });
  });
      ...
      function makeButton(cfg) {
        var vertical = cfg.vertical,
          itemText =
            ...
            (vertical ? sizeStyleVert : sizeStyle)[shrinkWrap] +
            '">&nbsp</div>',
          itemTextFirst =
            '<div style="display:inline-block;background:red;' +
            (vertical ? sizeStyleFirstVert : sizeStyleFirst)[shrinkWrap] +
            '">&nbsp</div>';
