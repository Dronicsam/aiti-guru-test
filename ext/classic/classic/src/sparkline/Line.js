...
Ext.define("Ext.sparkline.Line", {
  ...
  requires: ["Ext.sparkline.RangeMap"],
  ...
  renderHighlight: function (region) {
    ...
    if (spotRadius && highlightSpotColor) {
      canvas
        .drawCircle(vertex[0], vertex[1], spotRadius, null, highlightSpotColor)
        .append();
    }
  ...
  processRangeOptions: function () {
    ...
    if (
      chartRangeMin != null &&
      (me.chartRangeClip || chartRangeMin < me.miny)
    ) {
      me.miny = chartRangeMin;
    }
    if (
      chartRangeMax != null &&
      (me.chartRangeClip || chartRangeMax > me.maxy)
    ) {
      this.maxy = chartRangeMax;
    }
    if (
      chartRangeMinX != null &&
      (me.chartRangeClipX || chartRangeMinX < me.minx)
    ) {
      me.minx = chartRangeMinX;
    }
    if (
      chartRangeMaxX != null &&
      (me.chartRangeClipX || chartRangeMaxX > me.maxx)
    ) {
  ...
  renderGraph: function () {
    ...
    if (spotRadius) {
      ...
      if (
        hlSpotsEnabled ||
        minSpotColor ||
        (spotColor && yValues[yvallast] === me.miny)
      ) {
      ...
      if (
        hlSpotsEnabled ||
        maxSpotColor ||
        (spotColor && yValues[yvallast] === me.maxy)
      ) {
        canvasHeight -= Math.ceil(spotRadius);
      ...
      if (
        ...
        ((minSpotColor || maxSpotColor) &&
          (yValues[0] === me.miny || yValues[0] === me.maxy))
      ) {
    ...
    for (i = 0; i < plen; i++) {
      canvas
        .drawShape(lineShapes[i], me.getLineColor(), null, me.getLineWidth())
        .append();
    }
