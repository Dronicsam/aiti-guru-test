...
Ext.define("Ext.container.Container", {
  ...
  add: function () {
    ...
    for (i = 0; i < length; i++) {
      ...
      if (item.floating) {
        ...
        (!me.hasListeners.beforeadd ||
          me.fireEvent("beforeadd", me, item, pos) !== false) &&
  ...
  getChildByElement: function (el, deep) {
    ...
    for (; i < ln; i++) {
      ...
      if (itemEl && (itemEl.dom === el || itemEl.contains(el))) {
        return deep && item.getChildByElement
          ? item.getChildByElement(el, deep)
          : item;
      }
    }
    return null;
  ...
  remove: function (component, autoDestroy) {
    ...
    if (
      c &&
      (!me.hasListeners.beforeremove ||
        me.fireEvent("beforeremove", me, c) !== false)
    ) {
  ...
  privates: {
    ...
    applyDefaults: function (config) {
      ...
      if (defaults) {
        ...
        if (config.isComponent) {
          ...
          config = me
            .getConfigurator()
            .merge(me, Ext.Object.fork(defaults), config);
        }
      }
      return config;
    },
    ...
    doRemove: function (component, doDestroy) {
      ...
      doDestroy =
        doDestroy === true || (doDestroy !== false && this.autoDestroy);
