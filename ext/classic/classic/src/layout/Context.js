  ...
  debugHooks: {
    ...
    reportLayoutResult: function (layout, reported) {
      ...
      Ext.log(
        { indent: 1 },
        layout.done ? "++" : "--",
        me.getLayoutName(layout),
        ownerContext.isBoxParent ? " [isBoxParent]" : "",
        ownerContext.boxChildren
          ? " - boxChildren: " +
              ownerContext.state.boxesMeasured +
              "/" +
              ownerContext.boxChildren.length
          : "",
        ownerContext.boxParent
          ? " - boxParent: " + ownerContext.boxParent.id
          : "",
        " - size: ",
        ...
        for (key in me.layouts) {
          if (me.layouts.hasOwnProperty(key)) {
            ...
            if (me.items[layout.owner.el.id].isTopLevel) {
              if (
                me.reportOnSuccess ||
                me.layoutTreeHasFailures(layout, reported)
              ) {
