  ...
  "Ext.layout.ContextItem",
  {
    ...
    constructor: function (config) {
      ...
      if (!target.isComponent) {
        ...
        me.isComponentChild =
          target.ownerLayout && target.ownerLayout.isComponentLayout;
        ...
        if (
          ownerCt &&
          (ownerCtContext = ownerCt.el && me.context.items[ownerCt.el.id])
        ) {
        ...
        // logic exists there.
        me.sizeModel = sizeModel = target.getSizeModel(
          ownerCtContext &&
    ...
    initContinue: function (full) {
      ...
      if (full) {
        if (ownerCtContext && widthModel.shrinkWrap) {
          boxParent = ownerCtContext.isBoxParent
            ? ownerCtContext
            : ownerCtContext.boxParent;
          if (boxParent) {
    ...
    initAnimation: function () {
      ...
      if (ownerCtContext && ownerCtContext.isTopLevel) {
        ...
        !ownerCtContext &&
        target.isCollapsingOrExpanding &&
        target.animCollapse
      ) {
    ...
    flushAnimations: function () {
      ...
      if (animateFrom) {
        ...
        for (
          changeCount = 0, j = 0, propsLen = animateProps.length;
          j < propsLen;
          j++
        ) {
      ...
      if (!info) {
        if (!me.wrapsComponent) {
          ...
          if (!info) {
            ...
            info =
              me.parseMargins(comp, comp.margin) || me.checkCache("marginInfo");
    ...
    getDomProp: function (propName) {
      var me = this,
        result =
          me.dirty && propName in me.dirty ? undefined : me.props[propName];
    ...
    onBoxMeasured: function () {
      ...
      if (
        boxParent &&
        boxParent.widthModel.shrinkWrap &&
        !state.boxMeasured &&
        this.measuresBox
      ) {
    ...
    setContentSize: function (width, height, measured) {
      return (
        this.setContentWidth(width, measured) +
          this.setContentHeight(height, measured) ===
        2
      );
    },
    ...
    writeProps: function (dirtyProps, flushing) {
      ...
      for (propName in dirtyProps) {
        ...
        if (info && info.dom) {
          ...
          if (
            info.suffix &&
            (numericValue = parseInt(dirtyProps[propName], 10))
          ) {
      ...
      if (me.wrapsComponent && Ext.isIE9) {
        ...
        if (
          (hasWidth = width !== undefined && me.hasOverflowY) ||
      ...
      init: function () {
        ...
        if (me.context.logOn.initItem) {
          Ext.log(
            me.id,
            " consumers: content=",
            me.consumersContentWidth,
            "/",
            me.consumersContentHeight,
            ", container=",
            me.consumersContainerWidth,
            "/",
