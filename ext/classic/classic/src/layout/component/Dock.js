...
Ext.define("Ext.layout.component.Dock", {
  ...
  handleItemBorders: function () {
    ...
    if (lastItems) {
      for (i = 0, ln = lastItems.length; i < ln; i++) {
        ...
        if (
          !item.destroyed &&
          !item.ignoreBorderManagement &&
          !owner.manageBodyBorders
        ) {
    ...
    for (side in borders) {
      ...
      if (!owner.manageBodyBorders) {
        ...
        if (
          (!borders[side].satisfied && !owner.bodyBorder) ||
          owner.bodyBorder === false
        ) {
  ...
  beforeLayoutCycle: function (ownerContext) {
    ...
    ownerContext.shrinkWrapDockHeight =
      shrinkWrapDock & 1 && // jshint ignore:line
      ownerContext.heightModel.shrinkWrap;
    ownerContext.shrinkWrapDockWidth =
      shrinkWrapDock & 2 && // jshint ignore:line
      ownerContext.widthModel.shrinkWrap;
  },
  ...
  calculate: function (ownerContext) {
    ...
    for (
      forward = 0, backward = ownerContext.dockedItems.length;
      backward--;
      ++forward
    ) {
      if (horz) {
    ...
    if (
      horzDone &&
      vertDone &&
      me.finishConstraints(ownerContext, horzDone, vertDone)
    ) {
  ...
  createAxis: function (
    ...
    if (shrinkWrap) {
      ...
      begin = framing[dockBegin] + padding[dockBegin];
        ...
        ownerContext.getProp(sizeProp) - (framing[dockEnd] + padding[dockEnd]);
    }
  ...
  dockChild: function (ownerContext, axis, backward, forward) {
    var me = this,
      itemContext =
        ownerContext.dockedItems[axis.shrinkWrap ? backward : forward],
  ...
  finishConstraints: function (ownerContext, horz, vert) {
    ...
    if (widthModel || heightModel) {
      ...
      if (
        widthModel &&
        heightModel &&
        widthModel.constrainedMax &&
        heightModel.constrainedByMin
      ) {
      ...
      if (
        !ownerContext.widthModel.calculatedFromShrinkWrap &&
  ...
  getAnimatePolicy: function (ownerContext) {
    ...
    if (lastCollapsedState === "left" || lastCollapsedState === "right") {
      ...
      lastCollapsedState === "top" ||
      lastCollapsedState === "bottom"
    ) {
  ...
  getDockedItems: function (order, beforeBody) {
    ...
    if (sort) {
      ...
      Ext.Array.sort(dockedItems, function (a, b) {
        ...
        if (
          renderOrder &&
          (aw = me.owner.dockOrder[a.dock]) !==
            (bw = me.owner.dockOrder[b.dock])
        ) {
