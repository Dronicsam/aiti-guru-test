...
Ext.define("Ext.layout.container.boxOverflow.Scroller", {
  ...
  finishedLayout: function (ownerContext) {
    var me = this,
      ...
      names = layout.names,
      scrollPos = Math.min(
        me.getMaxScrollPosition(),
        ownerContext.innerCtScrollPos,
      ),
      lastProps;
    ...
    if (plan && plan.tooNarrow) {
      lastProps =
        ownerContext.childItems[ownerContext.childItems.length - 1].props;
  ...
  handleOverflow: function (ownerContext) {
    ...
    scrollerWidth =
      beforeScroller[getWidth]() +
      afterScroller[getWidth]() +
      beforeScroller.getMargin(parallelMargins) +
      afterScroller.getMargin(parallelMargins);

    targetPaddingWidth =
      ownerContext.targetContext.getPaddingInfo()[names.width];
  ...
  getBeforeScroller: function () {
    ...
    return (
      me._beforeScroller ||
      (me._beforeScroller = me.createScroller(
        me.beforeSuffix,
        "beforeRepeater",
        "scrollLeft",
      ))
  ...
  getAfterScroller: function () {
    ...
    return (
      me._afterScroller ||
      (me._afterScroller = me.createScroller(
        me.afterSuffix,
        "afterRepeater",
        "scrollRight",
      ))
  ...
  scrollTo: function (position, animate) {
    var me = this,
      ...
      newPosition = Ext.Number.constrain(
        position,
        0,
        me.getMaxScrollPosition(),
      );
    ...
    if (newPosition !== oldPosition && !me.scrolling) {
      ...
      layout.innerCt[names.scrollTo](
        names.beforeScrollX,
        newPosition,
        animate ? me.getScrollAnim() : false,
      );
      if (animate) {
        me.scrolling = true;
      } else {
