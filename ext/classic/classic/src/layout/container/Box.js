...
Ext.define("Ext.layout.container.Box", {
  ...
  beginLayoutCycle: function (ownerContext, firstCycle) {
    ...
    ownerContext.parallelSizeModel = widthModel =
      ownerContext[names.widthModel];
    ownerContext.perpendicularSizeModel = heightModel =
      ownerContext[names.heightModel];
  ...
  calculate: function (ownerContext) {
    ...
    if (state.parallelDone && state.perpendicularDone) {
      if (canScroll && !state.scrollPass) {
        if (
          needsScroll.parallel !== actualScroll.parallel ||
      ...
      if (
        me.done &&
        ownerContext.boxOptions.align.stretchmax &&
        !state.stretchMaxDone
      ) {
  ...
  calculatePerpendicular: function (ownerContext, names, plan) {
    ...
    if (isStretch) {
      ...
      for (i = 0; i < childItemsLength; i++) {
        ...
        childMargins = (childContext.marginInfo ||
          childContext.getMarginInfo())[heightName];
        ...
        if (
          !heightShrinkWrap &&
          constrain &&
          childContext[names.heightModel].shrinkWrap &&
    ...
    if (
      maxHeight > targetSize[heightName] &&
      canScroll &&
      canScroll.perpendicular
    ) {
    ...
    for (i = 0; i < childItemsLength; i++) {
      ...
      if (isStretch) {
        ...
        if (heightShrinkWrap && percentagePerpendicular) {
          childMargins =
            childContext.marginInfo || childContext.getMarginInfo();
          childHeight =
            percentagePerpendicular * height - childMargins[heightName];
  ...
  calculateStretchMax: function (ownerContext, names, plan) {
    ...
    for (i = 0; i < length; ++i) {
      ...
      if (
        childHeight !== props[heightName] || // if (wrong height ...
  ...
  publishInnerCtSize: function (ownerContext, widthOffset) {
    ...
    if (
      ...
      (plan.tooNarrow && state.canScroll)
    ) {
      innerCtWidth =
        state.contentWidth -
    ...
    if (align.stretch) {
      ...
      innerCtHeight =
        plan.maxSize +
        padding[names.beforeY] +
        padding[names.afterY] +
  ...
  applyOverflowHandler: function (overflowHandler, oldOverflowHandler) {
    ...
    if (
      oldOverflowHandler &&
      oldOverflowHandler.type === overflowHandler.type
    ) {
