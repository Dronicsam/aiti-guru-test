...
Ext.define(
  "Ext.layout.container.Auto",
  {
    ...
    calculate: function (ownerContext) {
      var me = this,
        ...
        calculatedItems =
          ...
          (state.calculatedItems = me.calculateItems
            ? me.calculateItems(ownerContext, containerSize)
            : true);
      ...
      if (
        calculatedItems &&
        ownerContext.hasDomProp("containerChildrenSizeDone")
      ) {
    ...
    calculateContentSize: function (ownerContext) {
      ...
      if (needed) {
        if (
          calcWidth &&
          !ownerContext.setContentWidth(me.measureContentWidth(ownerContext))
        ) {
          me.done = false;
        }
        if (
          calcHeight &&
          !ownerContext.setContentHeight(me.measureContentHeight(ownerContext))
        ) {
          me.done = false;
        }
    ...
    calculateOverflow: function (ownerContext) {
      ...
      if (xauto || yauto) {
        ...
        if (
          width !== me.lastOverflowAdjust.width ||
          height !== me.lastOverflowAdjust.height
        ) {
          me.done = false;
    ...
    getOverflowXStyle: function (ownerContext) {
      return (
        ...
        (ownerContext.overflowXStyle =
          ...
          ownerContext.overflowContext.getStyle("overflow-x"))
      );
    },
    ...
    getOverflowYStyle: function (ownerContext) {
      return (
        ownerContext.overflowYStyle ||
        (ownerContext.overflowYStyle =
          this.owner.scrollFlags.overflowY ||
          ownerContext.overflowContext.getStyle("overflow-y"))
      );
    },
    ...
    initContextItems: function (ownerContext) {
      ...
      ownerContext.overflowContext =
        overflowEl === ownerContext.el
          ? ownerContext
    ...
    initLayout: function () {
      ...
      if (
        scrollbarWidth &&
        me.manageOverflow &&
        !me.hasOwnProperty("lastOverflowAdjust")
      ) {
  ...
  function () {
    this.prototype.chromeCellMeasureBug =
      Ext.isChrome && Ext.chromeVersion >= 26;
  },
);
