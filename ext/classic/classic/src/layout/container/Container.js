...
Ext.define("Ext.layout.container.Container", {
  ...
  mixins: ["Ext.util.ElementContainer"],
  ...
  notifyOwner: function () {
    //<debug>
    if (
      !this._hasTargetWarning &&
      this.targetCls &&
      !this.getTarget().hasCls(this.targetCls)
    ) {
  ...
  getContainerSize: function (ownerContext, inDom) {
    ...
    if (!ownerContext.widthModel.shrinkWrap) {
      ++needed;
      width = inDom
        ? targetContext.getDomProp("width")
        : targetContext.getProp("width");
    ...
    if (!ownerContext.heightModel.shrinkWrap) {
      ++needed;
      height = inDom
        ? targetContext.getDomProp("height")
        : targetContext.getProp("height");
  ...
  getVisibleItems: function () {
    ...
    for (i = 0; i < ln; i++) {
      ...
      if (
        item.rendered &&
        this.isValidParent(item, target, i) &&
        item.hidden !== true &&
        !item.floated
      ) {
