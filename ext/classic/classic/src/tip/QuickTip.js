...
Ext.define("Ext.tip.QuickTip", {
  ...
  doTargetOver: function (target, xy, event) {
    ...
    if (
      me.activeTarget &&
      (target === me.activeTarget.el ||
        Ext.fly(me.activeTarget.el).contains(target))
    ) {
    ...
    if (target) {
      ...
      for (key in targets) {
        if (targets.hasOwnProperty(key)) {
          ...
          if (
            targetEl &&
            (targetEl.dom === target || targetEl.contains(target))
          ) {
  ...
  onTargetOut: function (e) {
    ...
    if (
      active &&
      e.within(me.activeTarget.el) &&
      !me.getTipCfg(e.getTarget(), e)
    ) {
      return;
    }
