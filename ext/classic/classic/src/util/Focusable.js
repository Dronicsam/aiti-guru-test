  ...
  "Ext.util.Focusable",
  {
    ...
    privates: {
      ...
      revertFocus: function () {
        ...
        if (!me.preventRefocus && focusEvent && hasFocus) {
          ...
          // The component canFocus, so we can simply focus its element.
            ...
            if (
              Ext.isIE8 ||
              (focusTarget.isFocusable && focusTarget.isFocusable())
            ) {
              focusTarget.focus();
            }
          }
      ...
      findFocusTarget: function () {
        ...
        for (
          owner = me.up(":not([disabled])");
          owner;
          owner = owner.up(":not([disabled])")
        ) {
          ...
          focusTargets = Ext.ComponentQuery.query(
            ":focusable:not([hasFocus])",
            owner,
          );
