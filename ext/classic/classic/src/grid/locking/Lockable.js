  ...
  "Ext.grid.locking.Lockable",
  {
    ...
    injectLockable: function () {
      ...
      var me = this,
        ...
        clipVertLockedScrollbar =
          scrollbarWidth > 0 && Ext.supports.touchScroll !== 2;
      ...
      lockedGrid.viewConfig = lockedViewConfig = lockedViewConfig
        ? Obj.chain(lockedViewConfig)
        : {};
      normalGrid.viewConfig = normalViewConfig = normalViewConfig
        ? Obj.chain(normalViewConfig)
        : {};
      ...
      normalGrid
        .getView()
        .getScrollable()
        .addPartner(lockedGrid.getView().getScrollable(), "y");
    ...
    processColumns: function (columns, lockedGrid) {
      ...
      for (i = 0, len = columns.length; i < len; ++i) {
        ...
        if (!column.headerId) {
          column.headerId =
            (column.initialConfig || column).id || "h" + ++me.headerCounter;
        }
      }
    ...
    afterLockedViewLayout: function () {
      var me = this,
        ...
        lockedViewHorizScrollBar =
          lockedView.scrollFlags.x && lockedGrid.headerCt.tooNarrow,
        normalViewHorizScrollBar =
          normalView.scrollFlags.x && normalGrid.headerCt.tooNarrow,
    ...
    onLockedViewMouseWheel: function (e) {
      ...
      if (!me.ignoreMousewheel) {
        if (lockedViewElDom) {
          ...
          verticalCanScrollDown =
            scrollTop !==
            lockedViewElDom.scrollHeight - lockedViewElDom.clientHeight;
          verticalCanScrollUp = scrollTop !== 0;
        }

        if (
          (deltaY < 0 && verticalCanScrollUp) ||
      ...
      if (normalScrollTop !== lockedScrollTop) {
        ...
        if (normalView.bufferedRenderer) {
          ...
          if (lockedRowContainer.dom) {
            ...
            lockedRowContainer.translate(
              null,
              normalView.bufferedRenderer.bodyTop,
            );
          }
        }
      }
    ...
    delaySyncLockedWidth: function () {
      ...
      if (!task) {
        task = me.syncLockedWidthTask = new Ext.util.DelayedTask(
          me.syncLockedWidth,
          me,
        );
      }
    ...
    syncLockedWidth: function () {
      ...
      if (normalColCount) {
        ...
        if (lockedColCount) {
          ...
          if (
            rendered &&
            locked.shrinkWrapColumns &&
            !locked.headerCt.forceFit
          ) {
            delete locked.flex;
            ...
            locked.setWidth(
              locked.headerCt.getTableWidth() + locked.gridPanelBorderWidth,
            );
          }
