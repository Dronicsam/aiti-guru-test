...
Ext.define(
  "Ext.grid.locking.View",
  {
    ...
    requires: ["Ext.view.AbstractView", "Ext.view.Table"],
    ...
    constructor: function (config) {
      ...
      config.normal.viewConfig.isNormalView =
        config.locked.viewConfig.isLockedView = true;
      ...
      config.locked.viewConfig.beforeLayout =
        config.normal.viewConfig.beforeLayout = me.beforeLayout;

      // Share the same NavigationModel
      config.locked.viewConfig.navigationModel =
      ...
      me.selModel = config.normal.viewConfig.selModel =
        lockedView.getSelectionModel();
      ...
      lockedView.lockingPartner =
        normalView =
        me.normalView =
          me.normalGrid.getView();
      normalView.lockingPartner = lockedView;

      me.loadMask =
        config.loadMask !== undefined ? config.loadMask : me.loadMask;
    ...
    onBindStore: function (store, initial, propName) {
      ...
      // call doFirstRefresh
      if (
        normalView.componentLayoutCounter &&
