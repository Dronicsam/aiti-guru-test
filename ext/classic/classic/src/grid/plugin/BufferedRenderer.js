...
Ext.define(
  "Ext.grid.plugin.BufferedRenderer",
  {
    ...
    onViewRefresh: function (view, records) {
      ...
      if (
        !view.componentLayoutCounter &&
        (view.headerCt.down("{flex}") || me.variableRowHeight)
      ) {
        view.on({
    ...
    onWrappedColumnWidthChange: function (oldWidth, newWidth) {
      ...
      if (me.store.getCount() && me.bodyTop) {
        ...
        me.setViewSize(
          Math.ceil(view.getHeight() / me.rowHeight) +
            me.trailingBufferZone +
            me.leadingBufferZone,
        );
        ...
        // Column got wider causing scroll range to shrink, leaving the view stranded past the end of the scroll range, position it back
          ...
          newWidth > oldWidth &&
          me.bodyTop + view.body.dom.offsetHeight - 1 > me.scrollHeight
        ) {
          me.setBodyTop(
            Math.max(0, me.scrollHeight - view.body.dom.offsetHeight),
          );
        }

        // The bodyTop calculated by renderRange left the body outside the viewport; center it on the viewport.
        else if (
          me.bodyTop > me.scrollTop ||
          me.bodyTop + view.body.dom.offsetHeight <
            me.scrollTop + view.getHeight(true)
        ) {
    ...
    stretchView: function (view, scrollRange) {
      ...
      if (me.scrollTop > scrollRange) {
        me.position = me.scrollTop = Math.max(
          scrollRange - view.body.dom.offsetHeight,
          0,
        );
    ...
    setViewSize: function (viewSize, fromLockingPartner) {
      var me = this,
        ...
        lockingPartner =
          me.view.lockingPartner && me.view.lockingPartner.bufferedRenderer,
      ...
      if (
        lockingPartner &&
        !fromLockingPartner &&
        lockingPartner.view.componentLayoutCounter
      ) {
    ...
    getViewRange: function () {
      ...
      if (store.data.getCount()) {
        return store.getRange(
          startIndex,
          startIndex + (me.viewSize || store.defaultViewSize) - 1,
        );
      } else {
        return [];
      }
    ...
    onReplace: function (store, startIndex, oldRecords, newRecords) {
      ...
      if (
        renderedSize &&
        lastAffectedIndex < rows.startIndex &&
        rows.getCount() >= me.viewSize
      ) {
      ...
      if (
        startIndex < rows.startIndex &&
        lastAffectedIndex <= rows.endIndex &&
        scrollIncrement
      ) {
        view.suspendEvent("scroll");
    ...
    scrollTo: function (recordIdx, options) {
      var args = arguments,
        ...
        lockingPartner =
          view.lockingPartner &&
          view.lockingPartner.grid.isVisible() &&
      ...
      if (record && (targetRow = view.getNode(record))) {
        view
          .getScrollable()
          .scrollIntoView(targetRow, null, animate, highlight);
      ...
      store.getRange(startIdx, endIdx, {
        callback: function (range, start, end) {
          ...
          me.position = me.scrollTop = Math.min(
            Math.max(0, tableTop - view.body.getOffsetsTo(targetRow)[1]) -
              scrollDecrement,
            scroller.getSize().y - viewDom.clientHeight,
          );
          if (lockingPartner) {
    ...
    handleViewScroll: function (direction) {
      ...
      // We scrolled close to the edge and the Store needs reloading
        ...
        if (
          me.variableRowHeight &&
          requestEnd === rows.endIndex &&
    ...
    bottomOfViewCloseToEdge: function () {
      ...
      if (me.variableRowHeight) {
        return (
          me.bodyTop + me.view.body.dom.offsetHeight <
          me.scrollTop + me.view.lastBox.height + me.numFromEdge * me.rowHeight
        );
      } else {
        return (
          me.view.all.endIndex - me.getLastVisibleRowIndex() < me.numFromEdge
        );
      }
    },
    ...
    topOfViewCloseToEdge: function () {
      ...
      if (me.variableRowHeight) {
        ...
        return (
          me.getFirstVisibleRowIndex() - me.view.all.startIndex < me.numFromEdge
        );
      }
    },
    ...
    refreshView: function (startIndex) {
      ...
      // Store doesn't fill the required view size. Simple start/end calcs.
        ...
        startIndex = Math.max(
          ...
          Math.min(
            startIndex,
            maxIndex - (viewSize - me.leadingBufferZone) + 1,
          ),
        );
    ...
    doRefreshView: function (range, startIndex, endIndex, options) {
      ...
      if (view.refreshCounter) {
        ...
        if (
          view.hasListeners.beforerefresh &&
        ...
        if (focusPosition && focusPosition.view === view) {
          ...
          if (
            focusPosition.rowIdx < startIndex ||
            focusPosition.rowIdx > endIndex
          ) {
            focusPosition = null;
          } else {
        ...
        if (range.length) {
          ...
          if (viewMoved) {
            ...
            if (previousFirstItem) {
              ...
              scrollIncrement =
                rows.last(true).offsetTop - previousLastItem.offsetTop;
            }
      ...
      if (!(start === rows.startIndex && end === rows.endIndex)) {
        ...
        if (store.rangeCached(start, end)) {
          ...
          if (me.synchronousRender || forceSynchronous) {
            ...
            if (!me.renderTask) {
              me.renderTask = new Ext.util.DelayedTask(
                me.onRangeFetched,
                me,
                null,
                false,
    ...
    onRangeFetched: function (range, start, end, options, fromLockingPartner) {
      ...
      if (newRows && lockingPartner && !lockingPartner.disabled) {
        ...
        partnerNewRows = lockingPartner.onRangeFetched(
          null,
          start,
          end,
          options,
      ...
      if (me.variableRowHeight) {
        ...
        // Scroll range not enough - add what we think will be enough to accommodate the final rows. Will be chopped when we get to the end. See above.
          ...
          me.stretchView(
            view,
            (me.scrollHeight +=
              (store.getCount() - view.all.endIndex) * me.rowHeight),
          );
        }
      }
    },
    ...
    getFirstVisibleRowIndex: function (
      ...
      if (rows.getCount() && me.variableRowHeight) {
        if (!arguments.length) {
          ...
          if (
            bodyTop > viewportBottom ||
            bodyTop + view.body.dom.offsetHeight < viewportTop
          ) {
            me.teleported = true;
        ...
        if (targetTop + elements[target].offsetHeight <= viewportTop) {
          return me.getFirstVisibleRowIndex(
            target + 1,
            endRow,
            viewportTop,
            viewportBottom,
    ...
    getLastVisibleRowIndex: function (
      ...
      if (rows.getCount() && me.variableRowHeight) {
        if (!arguments.length) {
          ...
          if (
            ...
            me.teleported = true;
            return (
              Math.floor(me.scrollTop / me.rowHeight) +
        ...
        if (targetTop > viewportBottom) {
          return me.getLastVisibleRowIndex(
            startRow,
            target - 1,
            viewportTop,
            viewportBottom,
      ...
      return (
        me.getFirstVisibleRowIndex() + Math.ceil(clientHeight / me.rowHeight)
      );
    },
    ...
    getScrollHeight: function () {
      ...
      if (!me.hasOwnProperty("rowHeight")) {
        ...
        if (rowCount) {
          me.rowHeight = me.variableRowHeight
            ? Math.floor(view.body.dom.clientHeight / rowCount)
            : rows.first(true).offsetHeight;
        }
      }
