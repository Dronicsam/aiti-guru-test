...
Ext.define("Ext.grid.plugin.RowExpander", {
  ...
  init: function (grid) {
    ...
    if (ownerLockable) {
      me.addExpander(
        ownerLockable.lockedGrid.headerCt.items.getCount()
          ? ownerLockable.lockedGrid
          : grid,
      );
      ...
      me.viewListeners = view.on({
        itemadd: Ext.Function.createAnimationFrame(
          ownerLockable.syncRowHeights,
          ownerLockable,
        ),
      });
    } else {
  ...
  setup: function (rows, rowValues) {
    ...
    if (
      lockable &&
      Ext.Array.indexOf(
        me.grid.columnManager.getColumns(),
        me.rowExpander.expanderColumn,
      ) !== -1
    ) {
  ...
  onKeyDown: function (view, record, row, rowIdx, e) {
    ...
    if (pos) {
      ...
      if (
        ((key === 107 || (key === 187 && e.shiftKey)) && isCollapsed) ||
  ...
  toggleRow: function (rowIdx, record) {
    ...
    if (me.expanderColumn) {
      expanderCell = Ext.fly(view.getRow(rowIdx)).down(
        me.expanderColumn.getCellSelector(),
        true,
      );
      if (expanderCell) {
  ...
  syncRowHeights: function (lockedItem, normalItem) {
    ...
    if (normalBd.isVisible()) {
      ...
      if (
        (lockedHeight = lockedBd.getHeight()) !==
        (normalHeight = normalBd.getHeight())
      ) {
  ...
  getHeaderConfig: function () {
    ...
    return {
      ...
      processEvent: function (
        ...
        if (
          (type === "click" &&
            e.getTarget("." + Ext.baseCSSPrefix + "grid-row-expander")) ||
