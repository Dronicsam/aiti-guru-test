...
Ext.define("Ext.grid.NavigationModel", {
  ...
  onCellMouseDown: function (
    ...
    if (
      view.actionableMode &&
      (mousedownEvent.getTarget(null, null, true).isTabbable() ||
        ((ac = Ext.ComponentManager.getActiveComponent()) &&
          ac.owns(mousedownEvent)))
    ) {
      return;
    }
  ...
  onItemMouseDown: function (view, record, item, index, mousedownEvent) {
    ...
    if (
      !mousedownEvent.position.cellElement &&
      (!parentEvent || parentEvent.type !== "touchstart")
    ) {
  ...
  afterStoreRemove: function (actionableMode, pos, store) {
    ...
    if (!pos.view.el.contains(Ext.Element.getActiveElement())) {
      ...
      pos = new Ext.grid.CellContext(view).setPosition(
        store.contains(pos.record)
          ? pos.record
          : Math.min(pos.rowIdx, view.dataSource.getCount() - 1),
        pos.colIdx,
      );
  ...
  setPosition: function (
    ...
    if (recordIndex && recordIndex.isCellContext) {
      ...
      if (dataSource.indexOf(newRecord) === -1) {
        ...
        if (
          scroller.getPosition().y >=
          scroller.getMaxPosition().y - view.all.last(true).offsetHeight
        ) {
          recordIndex.rowIdx--;
        }
        ...
        newColumnIndex = Math.min(
          newColumnIndex,
          columnManager.getColumns().length,
        );
    ...
    if (
      newRecordIndex === me.recordIndex &&
      newColumnIndex === me.columnIndex &&
  ...
  getPosition: function () {
    ...
    if (position.record && position.column) {
      ...
      if (
        curIndex === -1 ||
        view.getVisibleColumnManager().indexOf(position.column) === -1
      ) {
        position.setAll();
        me.record = me.column = me.recordIndex = me.columnIndex = null;
      } else {
  ...
  getLastFocused: function () {
    ...
    if (lastFocused && lastFocused.record && lastFocused.column) {
      ...
      if (
        view.dataSource.indexOf(lastFocused.record) !== -1 &&
  ...
  onKeyTab: function (keyEvent) {
    ...
    focusTarget =
      tabbableChildren[
        Ext.Array.indexOf(tabbableChildren, keyEvent.target) +
          (forward ? 1 : -1)
      ];
    ...
    while (
      !focusTarget &&
      (cell = cell[forward ? "nextSibling" : "previousSibling"])
    ) {
  ...
  onKeyDown: function (keyEvent) {
    ...
    var newRecord = keyEvent.record.isExpandingOrCollapsing
        ? null
        : keyEvent.view.walkRecs(keyEvent.record, 1),
      pos = this.getPosition();
  ...
  move: function (dir, keyEvent) {
    ...
    if (position && position.record) {
      ...
      return position.view.walkCells(
        position,
        dir,
