...
Ext.define("Ext.grid.selection.Rows", {
  ...
  remove: function (record) {
    ...
    if (
      me.selectedRecords &&
      me.selectedRecords.byInternalId.get(record.internalId)
    ) {
  ...
  getCount: function () {
    ...
    for (i = range[0]; i <= range[1]; i++) {
      if (
        !selectedRecords ||
        !selectedRecords.byInternalId.get(store.getAt(i).internalId)
      ) {
        result++;
      }
    }
  ...
  getFirstRowIndex: function () {
    return this.getCount()
      ? this.view.dataSource.indexOf(this.selectedRecords.first())
      : 0;
  },
      ...
      if (selection) {
        selection.each(function (record) {
          ...
          for (i = 0; i < colCount; i++) {
            ...
            if (
              fn.call(scope || me, context, context.colIdx, context.rowIdx) ===
              false
            ) {
              abort = true;
              return false;
  ...
  privates: {
    ...
    setRangeEnd: function (end) {
      ...
      for (
        rowIdx = Math.max(Math.min(range[0], lastRange[0]), rows.startIndex),
          end = Math.min(Math.max(range[1], lastRange[1]), rows.endIndex);
        rowIdx <= end;
        rowIdx++
      ) {
        ...
        if (rowIdx < range[0] || rowIdx > range[1]) {
          ...
          if (
            selected &&
            (rec = selected.byInternalId.get(store.getAt(rowIdx).internalId))
          ) {
            selected.remove(rec);
          }
    ...
    extendRange: function (extensionVector) {
      ...
      for (
        i = extensionVector.start.rowIdx;
        i <= extensionVector.end.rowIdx;
        i++
      ) {
    ...
    getContiguousSelection: function () {
      ...
      if (this.selectedRecords) {
        ...
        if (len) {
          for (i = 1; i < len; i++) {
            if (
              store.indexOf(selection[i]) !==
              store.indexOf(selection[i - 1]) + 1
            ) {
              return false;
            }
          }
