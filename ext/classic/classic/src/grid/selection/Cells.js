...
Ext.define("Ext.grid.selection.Cells", {
  ...
  getFirstRowIndex: function () {
    return this.startCell
      ? Math.min(this.startCell.rowIdx, this.endCell.rowIdx)
      : 0;
  },
  ...
  getLastRowIndex: function () {
    return this.startCell
      ? Math.max(this.startCell.rowIdx, this.endCell.rowIdx)
      : -1;
  },
  ...
  getFirstColumnIndex: function () {
    return this.startCell
      ? Math.min(this.startCell.colIdx, this.endCell.colIdx)
      : 0;
  },
  ...
  getLastColumnIndex: function () {
    return this.startCell
      ? Math.max(this.startCell.colIdx, this.endCell.colIdx)
      : -1;
  },
  ...
  privates: {
    ...
    extendRange: function (extensionVector) {
      ...
      if (extensionVector[extensionVector.type] < 0) {
        newEndCell = me.endCell
          .clone()
          .setPosition(me.getLastRowIndex(), me.getLastColumnIndex());
      ...
      if (start) {
        if (!start.colIdx && !start.rowIdx) {
          return (
              ...
              end.view.getVisibleColumnManager().getColumns().length - 1 &&
            end.rowIdx === end.view.dataSource.getCount - 1
          );
        }
      }
      return false;
