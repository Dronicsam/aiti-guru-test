  ...
  privates: {
    ...
    isAllSelected: function () {
      ...
      return (
        selectedColumns &&
        selectedColumns.length ===
          this.view.ownerGrid.getVisibleColumnManager().getColumns().length
      );
    },
    ...
    selectAll: function () {
      ...
      me.clear();
      me.selectedColumns =
        me.view.getSelectionModel().lastContiguousColumnRange = me.view
    ...
    extendRange: function (extensionVector) {
      ...
      for (
        i = extensionVector.start.colIdx;
        i <= extensionVector.end.colIdx;
        i++
      ) {
        me.add(columns[i]);
      }
    ...
    getContiguousSelection: function () {
      ...
      if (len) {
        for (i = 1; i < len; i++) {
          if (
            selection[i].getVisibleIndex() !==
            selection[i - 1].getVisibleIndex() + 1
          ) {
            return false;
          }
        }
