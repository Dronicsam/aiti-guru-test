...
Ext.define("Ext.grid.feature.Grouping", {
  ...
  init: function (grid) {
    ...
    view.addRowTpl(Ext.XTemplate.getTpl(me, "groupRowTpl")).groupingFeature =
      me;
    ...
    // If it's a local store we can build a grouped store for use as the view's dataSource
      ...
      if (lockPartner && lockPartner.dataSource) {
        ...
        me.dataSource =
          view.dataSource =
          dataSource =
            new Ext.grid.feature.GroupStore(me, store);
      }
    }
  ...
  enable: function () {
    ...
    groupToggleMenuItem = me.view.headerCt
      .getMenu()
  ...
  afterCollapseExpand: function (collapsed, groupName, focus) {
    ...
    if (focus) {
      if (header) {
        view.scrollElIntoView(
          Ext.fly(header).up(view.getItemSelector()),
          false,
          true,
        );
      }
    ...
    if (data.doGrouping) {
      ...
      if (record.isCollapsedPlaceholder) {
        ...
        metaGroupCache.name = metaGroupCache.renderedGroupValue = hasRenderer
          ? column.renderer(group.getAt(0).get(groupField), {}, record)
          : groupName;
      ...
      if (group) {
        ...
        rowValues.isLastRow =
          recordIndex ===
          (isBufferedStore ? store.getTotalCount() : store.getCount()) - 1;
      ...
      if (rowValues.isFirstRow) {
        metaGroupCache.groupField = groupField;
        metaGroupCache.name = metaGroupCache.renderedGroupValue = hasRenderer
          ? column.renderer(record.get(groupField), {}, record)
          : groupName;
  ...
  setup: function (rows, rowValues) {
    var me = this,
      ...
      isGrouping = (view.isGrouping =
        !me.disabled && me.getGridStore().isGrouped()),
      bufferedRenderer = view.bufferedRenderer;

    me.skippedRows = 0;
    if (bufferedRenderer) {
      bufferedRenderer.variableRowHeight =
        view.bufferedRenderer.variableRowHeight || isGrouping;
    }
  ...
  cleanup: function (rows, rowValues) {
    ...
    rowValues.metaGroupCache =
      rowValues.groupHeaderTpl =
  ...
  generateSummaryData: function () {
    ...
    for (i = 0, len = groups.length; i < len; ++i) {
      ...
      if (
        ...
        hasRemote ||
        store.updating ||
        me.didGroupChange(group, metaGroup, filters)
      ) {
