...
Ext.define("Ext.grid.feature.GroupStore", {
  ...
  processStore: function (store) {
    var me = this,
      ...
      groupNames =
        groups && ExtArray.unique(Ext.Object.getValues(groups.itemGroupKeys)),
      isCollapsed = false,
    ...
    if (store.getCount()) {
      ...
      if (groupCount > 0) {
        ...
        for (i = 0; i < groupCount; i++) {
          ...
          // to group.
          if (
            me.badGrouperKey === key &&
            (featureGrouper = groupingFeature.getGrouper(groupField))
          ) {
          ...
          metaGroup = metaGroupCache[key] =
            oldMetaGroupCache[key] || groupingFeature.getMetaGroup(key);
          ...
          isCollapsed = metaGroup.isCollapsed =
  ...
  getByInternalId: function (internalId) {
    ...
    return (
      this.store.getByInternalId(internalId) ||
      this.data.byInternalId.get(internalId)
    );
  },
  ...
  collapseGroup: function (group) {
    ...
    if (
      items &&
      (len = items.length) &&
      (startIdx = me.data.indexOf(items[0])) !== -1
    ) {
      // Any event handlers must see the new state
