...
Ext.define("Ext.grid.column.Widget", {
  ...
  onAdded: function () {
    ...
    if (view) {
      ...
      if (
        ...
        view.viewReady &&
        me.rendered &&
        view.getEl().down(me.getCellSelector())
      ) {
  ...
  privates: {
    getCachedStyle: function (el, style) {
      ...
      return (
        cachedStyles[style] ||
        (cachedStyles[style] = Ext.fly(el).getStyle(style))
      );
    },
    ...
    onViewRefresh: function (view, records) {
      ...
      if (me.isVisible(true)) {
        ...
        Ext.suspendLayouts();
        for (
          itemIndex = rows.startIndex, recordIndex = 0;
          itemIndex <= rows.endIndex;
          ...
          cell = view.getRow(rows.item(itemIndex)).cells[me.getVisibleIndex()]
            .firstChild;

          // Attempt to reuse the existing widget for this record.
          widget = me.liveWidgets[recordId] =
            oldWidgetMap[recordId] || me.getFreeWidget();
          widget.$widgetRecord = record;
          widget.$widgetColumn = me;
