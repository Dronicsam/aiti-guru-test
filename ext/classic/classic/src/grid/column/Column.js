...
Ext.define("Ext.grid.column.Column", {
  ...
  initComponent: function () {
    ...
    if (me.columns != null) {
      ...
      if (me.flex) {
        me.minWidth =
          me.minWidth || Ext.grid.plugin.HeaderResizer.prototype.minColWidth;
      }
    }
  ...
  bindFormatter: function (format) {
    ...
    return function (v) {
      return format.format(
        v,
        format.scope || me.rendererScope || me.resolveListenerScope(),
      );
    };
  },
  ...
  beforeRender: function () {
    ...
    if (
      ...
      !me.groupable &&
      !me.lockable &&
      (rootHeaderCt.grid.enableColumnHide === false ||
  ...
  onTitleElClick: function (e, t, sortOnClick) {
    ...
    if (e.pointerType === "touch") {
      ...
      if (
        !me.menuDisabled &&
        me.isAtEndEdge(e, parseInt(me.triggerEl.getStyle("width"), 10))
      ) {
        if (!me.menuDisabled) {
          activeHeader = me;
        }
      ...
      activeHeader =
        me.triggerEl &&
        (e.target === me.triggerEl.dom ||
          t === me.triggerEl ||
          e.within(me.triggerEl))
          ? me
          : null;
    }
    ...
    if (
      sortOnClick !== false &&
      ((!activeHeader && !me.isAtStartEdge(e) && !me.isAtEndEdge(e)) ||
        e.getKey())
    ) {
      me.toggleSortState();
    }
  ...
  hide: function () {
    ...
    if (owner.isGroupHeader) {
      ...
      if (
        me.isSubHeader &&
        !me.isGroupHeader &&
        owner.query(">:not([hidden])").length === 1
      ) {
  ...
  getCellWidth: function () {
    ...
    if (
      me.rendered &&
      me.componentLayout &&
      me.componentLayout.lastComponentSize
    ) {
  ...
  getCellInnerSelector: function () {
    return (
      this.getCellSelector() + " ." + Ext.baseCSSPrefix + "grid-cell-inner"
    );
  },
  ...
  isAtEndEdge: function (e, margin) {
    return (
      this.getX() + this.getWidth() - e.getXY()[0] <=
      (margin || this.handleWidth)
    );
  },
