...
Ext.define("Ext.grid.header.Container", {
  ...
  onHeaderCtEvent: function (e, t) {
    ...
    if (headerEl && !me.blockEvents) {
      ...
      if (header) {
        ...
        if (
          (!header.isGroupHeader && !header.isContainer) ||
          e.within(targetEl)
        ) {
  ...
  onAdd: function (c) {
    ...
    if (me._usedIDs[c.headerId]) {
      Ext.log.warn(
        this.$className + " attempted to reuse an existing id: " + c.headerId,
      );
    }
  ...
  move: function (fromIdx, toIdx) {
    ...
    headerToMove.visibleFromIdx = me
      .getRootHeaderCt()
      .visibleColumnManager.indexOf(headerToMove);
  ...
  getMenuItemForHeader: function (menu, header) {
    return header
      ? menu.down("menucheckitem[headerId=" + header.id + "]")
      : null;
  },
  ...
  onHeaderTriggerClick: function (header, e, t) {
    var me = this;
    if (
      header.fireEvent("headertriggerclick", me, header, e, t) !== false &&
  ...
  applyForceFit: function (header) {
    var me = this,
      ...
      availFlex =
        me.el.dom.clientWidth -
        (view.el.dom.scrollHeight > view.el.dom.clientHeight
