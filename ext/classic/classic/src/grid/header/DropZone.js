...
Ext.define("Ext.grid.header.DropZone", {
  ...
  positionIndicator: function (data, node, e) {
    ...
    if (
      !targetHeader.draggable &&
      pos === "before" &&
      targetHeader.getIndex() === 0
    ) {
      return false;
    }
    ...
    if (data.header.el.dom === node) {
      ...
      if (!doPosition && !from.ownerCt.sealed && !to.ownerCt.sealed) {
        ...
        if (fromPanel !== toPanel) {
          ...
          if (
            (data.isUnlock && from.lockable === false) ||
  ...
  onNodeDrop: function (node, dragZone, e, data) {
    ...
    // This is a drop within the same HeaderContainer.
      ...
      if (dropPosition === "before") {
        ...
        moveMethod =
          "move" +
          dropPosition.charAt(0).toUpperCase() +
          dropPosition.substr(1);
