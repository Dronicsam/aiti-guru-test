...
Ext.define("Ext.grid.RowEditor", {
  ...
  requires: ["Ext.tip.ToolTip", "Ext.util.KeyNav", "Ext.grid.RowEditorButtons"],
  ...
  initComponent: function () {
    ...
    if (me.lockable) {
      me.items = [
        ...
        (lockedCt = me.lockedColumnContainer =
          new Container({
        ...
        (normalCt = me.normalColumnContainer =
          new Container({
      ...
      lockedCt
        .getScrollable()
        .addPartner(grid.lockedGrid.view.getScrollable(), "x");
      normalCt
        .getScrollable()
        .addPartner(grid.normalGrid.view.getScrollable(), "x");
    } else {
  ...
  onGridResize: function () {
    ...
    me.wrapEl.setLocalX(
      gridBody.getOffsetsTo(grid)[0] +
        gridBody.getBorderWidth("l") -
    ...
    if (me.lockable) {
      me.lockedColumnContainer.setWidth(
        grid.lockedGrid.view.el.dom.clientWidth,
      );
    }
  },
  ...
  onColumnShow: function (column) {
    ...
    if (
      me.rendered &&
      !me.editingPlugin.reconfiguring &&
  ...
  onColumnMove: function (column, fromIdx, toIdx) {
    var me = this,
      ...
      fieldContainer = locked
        ? me.lockedColumnContainer
        : me.normalColumnContainer,
      columns,
      i,
      len,
      after,
  ...
  insertColumnEditor: function (column) {
    ...
    fieldContainer = column.isLocked()
      ? me.lockedColumnContainer
      : me.normalColumnContainer;
  ...
  onFieldTab: function (e) {
    var me = this,
      ...
      forwards = !e.shiftKey,
      target =
        activeField[forwards ? "nextNode" : "previousNode"](":focusable");
        ...
        if (rowIdx >= 0 && rowIdx <= me.view.dataSource.getCount()) {
          if (forwards) {
            ...
            activeField.column
              .getView()
              .getScrollable()
              .scrollIntoView(activeField.ownerCt.child(":focusable").el);
          } else {
  ...
  reposition: function (animateConfig, fromScrollHandler) {
    ...
    if (row && Ext.isElement(row)) {
      ...
      if (!me.editingPlugin.grid.rowLines) {
        ...
        yOffset = -parseInt(
          Ext.fly(row).first().getStyle("border-bottom-width"),
          10,
        );
      }
    ...
    if (context) {
      ...
      if (deltaY < 0) {
        ...
        deltaY = Math.max(
          deltaY +
            me.getHeight() +
            me.floatingButtons.getHeight() -
            scrollingViewDom.clientHeight -
            body.getBorderWidth("b"),
          0,
        );
        if (deltaY > 0) {
  ...
  calculateLocalRowTop: function (row) {
    ...
    return (
      Ext.fly(row).getOffsetsTo(grid)[1] -
      grid.el.getBorderWidth("t") +
      this.lastScrollTop
    );
  },
  ...
  getClientWidth: function () {
    ...
    if (me.lockable) {
      result =
        grid.lockedGrid.getWidth() + grid.normalGrid.view.el.dom.clientWidth;
    } else {
  ...
  renderColumnData: function (field, record, activeColumn) {
    var me = this,
      ...
      colIdx,
      scope =
        column.usingDefaultRenderer && !column.scope ? column : column.scope;
  ...
  syncButtonPosition: function (scrollDelta) {
    var me = this,
      ...
      overflow =
        me.getScrollDelta() -
        (scrollingView.getScrollable().getSize().y -
          scrollingView.getScrollY() -
          me.scrollingViewEl.dom.clientHeight);
  ...
  onHide: function () {
    ...
    if (me.activeField) {
      ...
      focusContext = new Ext.grid.CellContext(column.getView()).setPosition(
        me.context.record,
        column,
      );
