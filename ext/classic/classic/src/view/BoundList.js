...
Ext.define("Ext.view.BoundList", {
  ...
  onHide: function () {
    ...
    if (
      Ext.Element.getActiveElement() !== inputEl &&
  ...
  onItemClick: function (record) {
    ...
    if (!pickerField.multiSelect && selected.length) {
      ...
      if (
        selected &&
        pickerField.isEqual(record.get(valueField), selected.get(valueField)) &&
  ...
  onContainerClick: function (e) {
    ...
    if (
      this.pagingToolbar &&
      this.pagingToolbar.rendered &&
      e.within(this.pagingToolbar.el)
    ) {
      return false;
    }
  },
