...
Ext.define(
  "Ext.view.AbstractView",
  {
    ...
    refresh: function () {
      ...
      if (
        !me.hasListeners.beforerefresh ||
        me.fireEvent("beforerefresh", me) !== false
      ) {
    ...
    refreshSize: function (forceLayout) {
      ...
      if (
        sizeModel.height.shrinkWrap ||
        sizeModel.width.shrinkWrap ||
        forceLayout
      ) {
        me.updateLayout();
      }
        ...
        if (me.dataSource.getCount() === 0) {
          ...
          if (fireItemRemove) {
            me.fireEvent(
              ...
              records,
              index,
              me.getNodes(index, index + records.length - 1),
            );
          }
          me.refresh();
        } else {
    ...
    refreshView: function () {
      var me = this,
        ...
        blocked =
          me.blockRefresh ||
          !me.rendered ||
          me.up("[collapsed],[isCollapsingOrExpanding],[hidden]");
    ...
    privates: {
      ...
      createMask: function (mask) {
        ...
        if (
          maskStore &&
          !maskStore.isEmptyStore &&
          !maskStore.loadsSynchronously()
        ) {
          cfg = {
            target: me,
            msg: me.loadingText,
