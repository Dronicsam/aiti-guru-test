...
Ext.define("Ext.view.Table", {
  ...
  tpl: [
    ...
    '<div class="' +
      Ext.baseCSSPrefix +
      'grid-item-container" role="presentation" style="width:{fullWidth}px">',
    ...
    if (selModel) {
      if (selModel.isSelectionModel) {
        selModel.allowDeselect =
          grid.allowDeselect || selModel.selectionMode !== "SINGLE";
        ...
        // TODO: Remove selType config after deprecation period
          ...
          selModel.type =
            grid.selType || selModel.selType || selModel.type || defaultType;
        }
        if (!selModel.mode) {
  ...
  beforePageRemove: function (pageMap, pageNumber) {
    ...
    if (
      rows.startIndex >= (pageNumber - 1) * pageSize &&
  ...
  refreshSize: function (forceLayout) {
    ...
    if (!me.hasLoadingHeight) {
      ...
      if (
        forceLayout ||
        (me.hasVariableRowHeight() && me.dataSource.getCount())
      ) {
  ...
  clearViewEl: function (leaveNodeContainer) {
    ...
    if (me.hasListeners.itemremove) {
      for (i = all.startIndex; i <= all.endIndex; i++) {
        ...
        me.fireEvent(
          "itemremove",
          store.getByInternalId(item.getAttribute("data-recordId")),
          i,
          item,
          me,
        );
  ...
  renderCell: function (
    ...
    cellValues.tdCls =
      ...
      cellValues.style =
        "";
    cellValues.unselectableAttr = me.enableTextSelection
      ? ""
      : 'unselectable="on"';
    ...
    cellValues.value =
      value == null || value === "" ? column.emptyCellText : value;
    ...
    if (
      lastFocused &&
      lastFocused.record.id === record.id &&
      lastFocused.column === column
    ) {
  ...
  getCellInclusive: function (position, returnDom) {
    if (position) {
      var row = this.getRow(position.row),
        header = this.ownerCt
          .getColumnManager()
          .getHeaderAtIndex(position.column);
  ...
  getCellByPosition: function (position, returnDom) {
    if (position) {
      var view = position.view || this,
        ...
        header = position.column.isColumn
          ? position.column
          : view.getVisibleColumnManager().getHeaderAtIndex(position.column);
    ...
    if (!me.cellFocused && me.all.getCount() && me.dataSource.getCount()) {
      ...
      if (Ext.fly(focusTarget).is(me.getCellSelector())) {
        focusPosition = new Ext.grid.CellContext(me).setPosition(
          ...
          me.getHeaderByCell(focusTarget),
        );
      }
      ...
      // and that window is dismissed leading to automatic focus of the previously focused element.
        ...
        focusTarget !== me.el.dom
      ) {
        me.ownerGrid.setActionableMode(
          true,
          new Ext.grid.CellContext(me).setPosition(
            me.getRecord(focusTarget),
            me.getHeaderByCell(Ext.fly(focusTarget).up(me.getCellSelector())),
          ),
        );
      ...
      // We now have to decide which cell to focus
        ...
        if (focusPosition) {
          ...
          focusPosition = new Ext.grid.CellContext(
            me.isNormalView && me.lockingPartner.grid.isVisible()
              ? me.lockingPartner
              : me,
          ).setColumn(0);
        }
  ...
  getLastFocused: function () {
    ...
    if (lastFocused && lastFocused.record && lastFocused.column) {
      ...
      if (
        me.dataSource.indexOf(lastFocused.record) !== -1 &&
          ...
          if (column.preventUpdate) {
            ...
            if (cellUpdateFlag) {
              ...
              hasVariableRowHeight =
                hasVariableRowHeight || column.variableRowHeight;
            }
          }
        }
  ...
  processItemEvent: function (record, item, rowIndex, e) {
    var me = this,
      ...
      eventPosition = (e.position =
        me.eventPosition || (me.eventPosition = new Ext.grid.CellContext())),
      row,
      cell;
    ...
    for (i = 0; i < ln; i++) {
      ...
      if (feature.hasFeatureEvent) {
        ...
        if (featureTarget) {
          ...
          // fireEvent signature
          beforeArgs = feature.getFireEventArgs(
            "before" + prefix + type,
            me,
            featureTarget,
            e,
  ...
  walkCells: function (pos, direction, verifierFn, scope) {
    var me = this,
      result = pos.clone(),
      lockingPartner =
        me.lockingPartner && me.lockingPartner.grid.isVisible()
          ? me.lockingPartner
          : null,
      rowIdx = pos.rowIdx,
  ...
  walkRecs: function (startRec, distance) {
    ...
    var me = this,
      ...
      limit =
        distance < 0
          ? 0
          : (store.isBufferedStore ? store.getTotalCount() : store.getCount()) -
            1,
      increment = limit ? 1 : -1,
    ...
    do {
      ...
      if (
        !rec.isCollapsedPlaceholder &&
        (node = Ext.fly(me.getNodeByRecord(rec))) &&
        node.isVisible(true)
      ) {
        moved += increment;
        lastValid = rec;
  ...
  privates: {
    ...
    setActionableMode: function (enabled, position) {
      ...
      if (enabled) {
        if (
          ...
          if (!lockingPartner) {
            focusCell = Ext.fly(focusRow).down(
            ...
            for (i = 0; i < len; i++) {
              isActionable =
                isActionable || actionables[i].activateCell(position);
            }
          }
        ...
        if (me.el.contains(activeEl)) {
          navModel.setPosition(
            new Ext.grid.CellContext(me).setPosition(
              me.getRecord(activeEl) || 0,
              me.getHeaderByCell(activeEl.findParent(me.getCellSelector())) ||
                0,
            ),
          );
        }
    ...
    onRowExit: function (prevRow, newRow, forward) {
      ...
      if (lockingPartner && lockingPartner.grid.isVisible()) {
        ...
        // TAB out of left side of view
          ...
          cellIdx =
            lockingPartner.getVisibleColumnManager().getColumns().length - 1;
    ...
    findFirstActionableElement: function (focusRow, direction, forward) {
      ...
      if (focusRow) {
        ...
        for (
          i = forward ? 0 : columnCount - 1;
          (forward ? i < columnCount : i > -1) && !focusTarget;
          i = i + (forward ? 1 : -1)
        ) {
          column = columns[i];
          ...
          for (j = 0; j < actionableCount; j++) {
            isActionable =
              isActionable || actionables[j].activateCell(position);
          }
