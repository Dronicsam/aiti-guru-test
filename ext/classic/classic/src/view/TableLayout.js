...
Ext.define("Ext.view.TableLayout", {
  ...
  beginLayout: function (ownerContext) {
    ...
    if (partner && partner.grid.isVisible()) {
      if (!ownerContext.lockingPartnerContext) {
        (ownerContext.lockingPartnerContext =
          context.getCmp(partner)).lockingPartnerContext = ownerContext;
      }
    ...
    // reach us as well):
    (ownerContext.headerContext = context.getCmp(me.headerCt)).viewContext =
      ownerContext;
  },
  ...
  calculate: function (ownerContext) {
    ...
    if (columnsChanged) {
      if (!(columnFlusher = state.columnFlusher)) {
        ...
        context.queueFlush(
          (state.columnFlusher = columnFlusher =
            {
    ...
    if (lockingPartnerContext) {
      if (!(rowHeightFlusher = state.rowHeightFlusher)) {
        ...
        if (
          !(otherSynchronizer = lockingPartnerContext.getProp("rowHeights"))
        ) {
          me.done = false;
          return;
        }
        ...
        context.queueFlush(
          (state.rowHeightFlusher = rowHeightFlusher =
            {
              ownerContext: ownerContext,
  ...
  flushColumnWidths: function () {
    ...
    for (i = 0; i < len; i++) {
      ...
      owner.body
        .select(owner.getColumnSizerSelector(column.target))
        .setWidth(colWidth);
