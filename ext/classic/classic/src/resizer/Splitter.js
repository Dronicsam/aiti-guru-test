...
Ext.define("Ext.resizer.Splitter", {
  ...
  onRender: function () {
    ...
    if (collapseEl) {
      collapseEl.lastCollapseDirCls =
        me.collapseDirProps[me.collapseDirection].cls;
    }
  },
    ...
    if (!dir) {
      ...
      if (!dir) {
        ...
        if (collapseTarget.isComponent) {
          ...
          idx =
            (Number(items.indexOf(collapseTarget) === items.indexOf(me) - 1) <<
              1) |
            Number(type === "hbox");
        } else {
          idx =
            (Number(me.collapseTarget === "prev") << 1) |
            Number(type === "hbox");
        }
  ...
  applyOrientation: function () {
    ...
    if (
      !me.hasOwnProperty(stretchSizeProp) ||
      me[stretchSizeProp] === defaultSize
    ) {
