...
Ext.define("Ext.form.field.ComboBox", {
  ...
  applyValueNotFoundText: function (v) {
    var me = this,
      valueNotFoundRecord =
        me.valueNotFoundRecord ||
        (me.valueNotFoundRecord = new Ext.data.Model());
    ...
    if (me.forceSelection) {
      if (me.multiSelect) {
        ...
        if (rec) {
          ...
          if (
            me.getDisplayValue([me.getRecordDisplayData(rec)]) !== displayValue
          ) {
            me.select(rec, true);
          }
        } else if (lastRecords) {
  ...
  doAutoSelect: function () {
    ...
    if (picker && me.autoSelect && me.store.getCount() > 0) {
      ...
      picker
        .getNavigationModel()
        .setPosition(me.picker.getSelectionModel().lastSelected || 0);
    }
  },
  ...
  onFieldMutation: function (e) {
    ...
    if (
      !me.readOnly &&
      (rawValue !== me.lastMutatedValue || isDelete) &&
      key !== e.TAB
    ) {
  ...
  onBeforePickerShow: function (picker) {
    ...
    var me = this,
      ...
      heightBelow =
        Ext.Element.getViewportHeight() - heightAbove - me.getHeight();
  ...
  doSetValue: function (value /* private for use by addValue */, add) {
    ...
    value = add
      ? Ext.Array.from(me.value).concat(value)
