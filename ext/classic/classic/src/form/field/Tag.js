...
Ext.define("Ext.form.field.Tag", {
  ...
  requires: ["Ext.selection.Model", "Ext.data.Store", "Ext.data.ChainedStore"],
   ...
   */
  fieldSubTpl: [
    '<div id="{cmpId}-listWrapper" data-ref="listWrapper" class="' +
      Ext.baseCSSPrefix +
      'tagfield {fieldCls} {typeCls} {typeCls}-{ui}" style="{wrapperStyle}">',
    '<ul id="{cmpId}-itemList" data-ref="itemList" class="' +
      Ext.baseCSSPrefix +
      'tagfield-list{itemListCls}">',
    '<li id="{cmpId}-inputElCt" data-ref="inputElCt" class="' +
      Ext.baseCSSPrefix +
      'tagfield-input">',
    ...
    'class="' +
      Ext.baseCSSPrefix +
      'tagfield-input-field {inputElCls}" autocomplete="off">',
    "</li>",
    "</ul>",
    "</div>",
    {
  ...
  onKeyDown: function (e) {
    ...
    if (
      valueCollection.getCount() > 0 &&
      (rawValue === "" ||
        (me.getCursorPosition() === 0 && !me.hasSelectedText()))
    ) {
  ...
  onKeyUp: function (e, t) {
    ...
    if (
      (me.multiSelect &&
        me.delimiterRegexp &&
        me.delimiterRegexp.test(rawValue)) ||
  ...
  getMultiSelectItemMarkup: function () {
    ...
    if (!me.multiSelectItemTpl) {
      ...
      me.multiSelectItemTpl = new Ext.XTemplate([
        '<tpl for=".">',
        '<li data-selectionIndex="{[xindex - 1]}" data-recordId="{internalId}" class="' +
          me.tagItemCls +
          childElCls,
  ...
  getRecordByListItemNode: function (itemEl) {
    return this.valueCollection.items[
      Number(itemEl.getAttribute("data-selectionIndex"))
    ];
  },
