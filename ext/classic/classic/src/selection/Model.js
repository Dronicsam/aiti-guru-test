...
Ext.define("Ext.selection.Model", {
  ...
  applySelected: function (selected) {
    if (!selected.isCollection) {
      selected = new Ext.util.Collection(
        Ext.apply(
          {
            rootProperty: "data",
          },
  ...
  onNavigate: function (e) {
    ...
    var me = this,
      ...
      isSelected = me.isSelected(record),
      from =
        me.selectionStart && me.isSelected(e.previousRecord)
          ? me.selectionStart
      ...
      direction =
        key === keyEvent.UP || key === keyEvent.PAGE_UP
          ? "up"
          : key === keyEvent.DOWN || key === keyEvent.DOWN
            ? "down"
            : null;
  ...
  refresh: function () {
    ...
    if (
      !store ||
      !(selected.isCollection || selected.isRows) ||
      !selected.getCount()
    ) {
      return;
    }
  ...
  onSelectChange: function (record, isSelected, suppressEvent, commitFn) {
    ...
    if (
      (suppressEvent || me.fireEvent("before" + eventName, me, record)) !==
        false &&
      commitFn() !== false
    ) {
      if (!suppressEvent) {
