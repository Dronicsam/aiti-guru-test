  ...
  function () {
      ...
      Ext.apply(styleHooks.opacity, {
        ...
        set: function (dom, value) {
          ...
          if (typeof value === "number" && value >= 0 && value < 1) {
            value *= 100;
            style.filter =
              val + (val.length ? " " : "") + "alpha(opacity=" + value + ")";
          } else {
            style.filter = val;
          }
        },
      ...
      proto.is = function (selector) {
        ...
        if (!(testFn = Ext.isFunction(selector) ? selector : cache[selector])) {
          if (!(match = selector.match(simpleSelectorRe))) {
            ...
            is =
              Ext.Array.indexOf(Ext.fly(root, "_is").query(selector), dom) !==
              -1;
          ...
          tag = match[1];
          cls = match[2];
          cache[selector] = testFn = cls
            ? classMatcher(tag, cls)
            : tagMatcher(tag);
        }
    ...
    if (Ext.isIE8) {
      ...
      while (k--) {
        ...
        styleHooks["border-" + edge.toLowerCase() + "-width"] = styleHooks[
          borderWidth
        ] = {
          name: borderWidth,
    ...
    Ext.onInternalReady(
      function () {
                ...
                if (pxRe.test(value)) {
                  ...
                  if (frameInfo) {
                    frameBodyStyle.height =
                      parseInt(value, 10) - frameInfo.height + "px";
                  }
        ...
        function fixRightMargin(dom, el, inline, style) {
          ...
          if (result !== "0px") {
            ...
            result = (
              inline
                ? style
                : dom.ownerDocument.defaultView.getComputedStyle(dom, null)
            ).marginRight;
        ...
        function fixRightMarginAndInputFocus(dom, el, inline, style) {
          ...
          if (result !== "0px") {
            ...
            result = (
              inline
                ? style
                : dom.ownerDocument.defaultView.getComputedStyle(dom, "")
            ).marginRight;
            domStyle.display = display;
            cleaner();
          }
        ...
        proto.verticalStyleHooks90 = verticalStyleHooks90 =
          Ext.Object.chain(styleHooks);
        proto.verticalStyleHooks270 = verticalStyleHooks270 =
          Ext.Object.chain(styleHooks);
