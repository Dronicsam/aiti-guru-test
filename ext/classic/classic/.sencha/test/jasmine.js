...
var xdescribe = function (description, specDefinitions, coverageFile) {
  return jasmine
    .getEnv()
    .describe(description, specDefinitions, coverageFile)
    .disable();
};
...
jasmine.Env.prototype.xspecFor = function (
  object,
  specForFn,
...
jasmine.Matchers.prototype.toRaiseExtError = function (expected) {
  ...
  if (extError && extError instanceof Error) {
    result =
      expected === jasmine.undefined ||
      this.env.contains_(extError.toString(), expected.message || expected);
  }
...
jasmine.Matchers.prototype.hasHTML = function (expected) {
  ...
  var normalizedHTML = me.actual.innerHTML.replace(
    /<[^>]*>/g,
    function (match1) {
  ...
  me.message = function () {
    return [
      "Expected dom element innerHTML to be " +
        expected +
        " but was " +
        normalizedHTML,
...
jasmine.Matchers.prototype.toBePositionedAt = function (x, y) {
  ...
  this.message = function () {
    return (
      "Expected Ext.Element to be positioned at (" +
      x +
      "," +
      y +
      ") but was positioned at (" +
      xy[0] +
      "," +
      xy[1] +
      ")"
...
(function () {
          ...
          Ext.Object.each(value, function (prop, expected) {
            ...
            if (Ext.isArray(expected)) {
              if (actual < expected[0] || actual > expected[1]) {
                expect(pfx + "=" + actual).toBe(
                  "in [" + expected[0] + "," + expected[1] + "]",
                );
              }
  ...
  jasmine.Matchers.prototype.toBeGE = jasmine.Matchers.prototype.toBeAtLeast =
  ...
  jasmine.Matchers.prototype.toBeGT =
    jasmine.Matchers.prototype.toBeGreaterThan;
})();
  ...
  prototype.format = function (value) {
    ...
    if (value) {
      ...
      if (className !== undefined) {
        ...
        this.emitScalar(
          className + "#" + (value.id || (value.getId && value.getId())),
        );
        return;
      }
    }
...
(function () {
  ...
  proto.checkDomLeak = function () {
    ...
    if (badNodes.length) {
      ...
      this.fail(
        "document.body contains childNodes after spec execution --> " +
          this.getFullName(),
      );
    }
  };
  ...
  proto.checkComponentLeak = function (spec) {
    ...
    if (leaks.length) {
      ...
      this.fail(
        "Ext.ComponentMgr reports undestroyed components after spec execution",
      );
    }
  };
  ...
  proto.collectGarbage = function () {
    ...
    if (ids.length) {
      this.fail(
        "Orphan Ext.dom.Element(s) detected: '" + ids.join("', '") + "'",
      );
    }
  };
...
(function () {
  ...
  jasmine.Suite = function () {
    ...
    this.totalSpecs = 0;
    this.fileName = jasmine.getCurrentScript();
    this.id = jasmine.hashString(
      this.getFullName(),
      parentSuite ? parentSuite.id : 0,
    );
  };
...
jasmine.WaitsForBlock.prototype.execute = function (onComplete) {
  ...
  try {
    latchFunctionResult = this.latchFunction.call(
      this.spec,
      this.timeout,
...
jasmine.pointerEventsMap =
  Ext.supports.MSPointerEvents && !Ext.supports.PointerEvents
    ? {
 ...
 */
jasmine.fireMouseEvent = function (
  target,
  type,
  x,
  y,
  ...
  if (Ext.isIE9m && doc.createEventObject) {
    ...
    body = doc.body;
    x =
      x + ((docEl && docEl.clientLeft) || 0) + ((body && body.clientLeft) || 0);
    y =
      y + ((docEl && docEl.clientTop) || 0) + ((body && body.clientLeft) || 0);
    Ext.apply(e, {
      bubbles: true,
      cancelable: true,
      screenX: x,
...
jasmine.doFireMouseEvent = function (
  target,
  type,
  x,
  y,
 ...
 */
jasmine.firePointerEvent = function (
  target,
  type,
  pointerId,
  x,
 ...
 * @return {Boolean} true if the event was successfully dispatched
 */
jasmine.fireTouchEvent = function (
  target,
  type,
  touches,
  changedTouches,
  ...
  Ext.apply(e, {
    ...
    changedTouches: jasmine.createTouchList(
      changedTouches ? changedTouches : touches,
      target,
    ),
    targetTouches: jasmine.createTouchList(
...
jasmine.simulateArrowKey =
  jasmine.syncPressArrowKey =
  jasmine.simulateKey =
  jasmine.syncPressKey =
...
jasmine.waitForFocus = jasmine.waitsForFocus = function (cmp, desc, timeout) {
  var isComponent = cmp.isComponent,
    dom = isComponent ? cmp.getFocusEl().dom : cmp.isElement ? cmp.dom : cmp,
...
jasmine.pressArrowKey =
  jasmine.asyncPressArrowKey =
  jasmine.pressKey =
  jasmine.asyncPressKey =
    function (from, key, options) {
...
jasmine.expectFocused = jasmine.expectsFocused = function (want, noWait) {
  ...
  runs(function () {
    var have = want.isComponent
      ? Ext.ComponentManager.getActiveComponent()
      : want.isElement
        ? Ext.fly(document.activeElement)
...
var MockAjaxManager = {
  ...
  removeMethods: function () {
    ...
    Ext.data.request.Ajax.prototype.getXhrInstance =
      MockAjaxManager.getXhrInstance;
