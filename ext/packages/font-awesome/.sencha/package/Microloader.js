      ...
      Microloader = {
          ...
          if (typeof manifest === "string") {
            ...
            if (content) {
              ...
              Boot.fetch(url, function (result) {
                //<debug>
                _debug(
                  "Manifest file was not found in Local Storage, loading: " +
                    url,
                );
                //</debug>
                manifest = new Manifest({
        ...
        load: function (manifest) {
          ...
          for (len = assets.length, i = 0; i < len; i++) {
            ...
            include = true;
            if (
              asset.assetConfig.platform &&
              !Boot.filterPlatform(asset.assetConfig.platform)
            ) {
              include = false;
            }
            if (include) {
          ...
          if (!result.error) {
            ...
            if (!Microloader.manifest.is(manifest)) {
              ...
              for (prop in newAssets) {
                ...
                include = !(
                  newAsset.assetConfig.platform &&
                  !Boot.filterPlatform(newAsset.assetConfig.platform)
                );
