...
(Ext.env || (Ext.env = {})).Browser = function (userAgent, publish) {
  ...
  var me = this,
    ...
    browserMatch = userAgent.match(
      new RegExp(
        "((?:" +
          Ext.Object.getValues(browserPrefixes).join(")|(?:") +
          "))([\\w\\._]+)",
      ),
    ),
    engineMatch = userAgent.match(
      new RegExp(
        "((?:" +
          Ext.Object.getValues(enginePrefixes).join(")|(?:") +
          "))([\\w\\._]+)",
      ),
    ),
  ...
  if (browserMatch) {
    browserName =
      browserNames[Ext.Object.getKey(browserPrefixes, browserMatch[1])];
  ...
  if (browserVersion) {
    ...
    if (me.is.IE) {
      ...
      if (
        mode === 7 ||
        (majorVer === 7 && mode !== 8 && mode !== 9 && mode !== 10)
      ) {
        majorVer = 7;
      } else if (
        mode === 8 ||
        (majorVer === 8 && mode !== 8 && mode !== 9 && mode !== 10)
      ) {
        majorVer = 8;
      } else if (
        mode === 9 ||
        (majorVer === 9 && mode !== 7 && mode !== 8 && mode !== 10)
      ) {
        majorVer = 9;
      } else if (
        mode === 10 ||
        (majorVer === 10 && mode !== 7 && mode !== 8 && mode !== 9)
      ) {
        majorVer = 10;
      } else if (
        mode === 11 ||
  ...
  if (
    /(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)(?!.*FBAN)/i.test(userAgent)
  ) {
    isWebView = true;
  }
  ...
  this.identity =
    browserName + majorVer + (this.isStrict ? "Strict" : "Quirks");
};
...
Ext.env.Browser.prototype = {
  ...
  getPreferredTranslationMethod: function (config) {
    if (
      ...
      "translationMethod" in config &&
      config.translationMethod !== "auto"
    ) {
...
(function (userAgent) {
   ...
   */
  Ext.SSL_SECURE_URL =
    Ext.isSecure && Ext.isIE ? "javascript:''" : "about:blank"; // jshint ignore:line
