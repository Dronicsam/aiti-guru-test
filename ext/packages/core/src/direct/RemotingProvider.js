...
Ext.define("Ext.direct.RemotingProvider", {
  ...
  constructor: function (config) {
    ...
    me.namespace = Ext.isString(me.namespace)
      ? Ext.ns(me.namespace)
      : me.namespace || Ext.global;
  ...
  onData: function (options, success, response) {
    ...
    if (success) {
      ...
      for (i = 0, len = events.length; i < len; ++i) {
        ...
        if (
          transaction &&
          me.fireEvent("beforecallback", me, event, transaction) !== false
        ) {
  ...
  getTransaction: function (options) {
    return options && options.tid
      ? Ext.direct.Manager.getTransaction(options.tid)
      : null;
  },
  ...
  sendRequest: function (transaction) {
    ...
    if (enableUrlEncode) {
      params = {};
      params[Ext.isString(enableUrlEncode) ? enableUrlEncode : "data"] =
        Ext.encode(callData);
