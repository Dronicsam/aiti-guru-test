      ...
      if (me.findListener(fn, scope) === -1) {
        ...
        if (priority) {
          ...
          if (!isNegativePriority || hasNegativePriorityIndex) {
            ...
            for (
              i = isNegativePriority ? highestNegativePriorityIndex : 0;
              i < length;
              i++
            ) {
              // Listeners created without options will have no "o" property
              listenerPriority = listeners[i].o
                ? listeners[i].o.priority || 0
                : 0;
        ...
        if (manager) {
          ...
          if (managedListeners) {
            for (i = managedListeners.length; i--; ) {
              ...
              if (
                managedListener.item === me.observable &&
                managedListener.ename === eventName &&
                managedListener.fn === fn &&
                managedListener.scope === scope
              ) {
          ...
          if (options) {
            ...
            if (delegate) {
              if (isElement) {
                ...
                isComponent &&
                !firingObservable.is(
                  "#" + observable.id + " " + options.delegate,
                )
              ) {
                continue;
              }
