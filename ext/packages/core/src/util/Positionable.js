...
Ext.define("Ext.util.Positionable", {
  ...
  calculateAnchorXY: function (anchor, extraX, extraY, mySize) {
    ...
    myWidth =
      mySize.width ||
      (isViewport ? Ext.Element.getViewportWidth() : me.getWidth());
    myHeight =
      mySize.height ||
      (isViewport ? Ext.Element.getViewportHeight() : me.getHeight());
  ...
  getAlignToXY: function (alignToEl, posSpec, offset) {
    ...
    if (constrain) {
      ...
      swapY =
        x < alignToElRegion.right &&
        x + myWidth >= alignToElRegion.left &&
        ((p1y == "t" && p2y == "b") || (p1y == "b" && p2y == "t"));
      ...
      swapX =
        y < alignToElRegion.bottom &&
        y + myHeight >= alignToElRegion.top &&
        ((p1x == "r" && p2x == "l") || (p1x == "l" && p2x == "r"));
  ...
  calculateConstrainedPosition: function (
    ...
    vector = (
      me.constrainHeader && me.header.rendered ? me.header : me
    ).getConstrainVector(constrainTo, proposedConstrainPosition, proposedSize);
  ...
  getConstrainVector: function (constrainTo, proposedPosition, proposedSize) {
    var thisRegion = this.getRegion(),
      vector = [0, 0],
      shadowSize =
        this.shadow && this.constrainShadow && !this.shadowDisabled
          ? this.shadow.getShadowSize()
          : undefined,
      overflowed = false,
      constrainSize,
