      ...
      if (instanceConfig) {
        for (name in instanceConfig) {
          ...
          if (!cfg) {
            //<debug>
            if (
              instance.$configStrict &&
              typeof instance.self.prototype[name] === "function"
            ) {
    ...
    reconfigure: function (instance, instanceConfig, options) {
      ...
      for (name in instanceConfig) {
        ...
        if (cfg) {
          ...
          if (name !== "type") {
            Ext.log.error(
              'No such config "' + name + '" for class ' + instance.$className,
            );
          }
          //</debug>
          continue;
