...
Ext.Loader = new (function () {
    ...
    addUsedClasses: function (classes) {
      ...
      if (classes) {
        ...
        for (i = 0, ln = classes.length; i < ln; i++) {
          ...
          if (
            typeof cls === "string" &&
            !Ext.Array.contains(usedClasses, cls)
          ) {
  ...
  Class.registerPreprocessor(
    ...
    function (cls, data, hooks, continueFn) {
      ...
      Ext.classSystemMonitor &&
        Ext.classSystemMonitor(cls, "Ext.Loader#loaderPreprocessor", arguments); // jshint ignore:line
      //</debug>
      ...
      (className ? Loader.exclude(className) : Loader).require(
        dependencies,
        function () {
          for (i = 0, ln = dependencyProperties.length; i < ln; i++) {
            propertyName = dependencyProperties[i];
  ...
  Manager.registerPostprocessor("uses", function (name, cls, data) {
    //<debug>
    Ext.classSystemMonitor &&
      Ext.classSystemMonitor(cls, "Ext.Loader#usesPostprocessor", arguments); // jshint ignore:line
    //</debug>
