...
Ext.define("Ext.scroll.TouchScroller", {
  ...
  applyIndicators: function (indicators, oldIndicators) {
    ...
    if (indicators) {
      ...
      if (oldIndicators) {
        ...
        if (xIndicator) {
          indicators.x = new Ext.scroll.Indicator(
            Ext.applyIf(
              {
                axis: "x",
                scroller: me,
        ...
        if (yIndicator) {
          indicators.y = new Ext.scroll.Indicator(
            Ext.applyIf(
              {
                axis: "y",
                scroller: me,
  ...
  applyInnerElement: function (innerElement) {
    ...
    if (this.isConfiguring && !innerElement) {
      Ext.raise(
        "Cannot create Ext.scroll.TouchScroller instance with null innerElement",
      );
    }
    //</debug>
  ...
  privates: {
      ...
      if (positionChanged) {
        if (animation) {
          ...
          // They want a callback, so we need to create a sequence on it.
            ...
            animation.callback = Ext.Function.createSequence(
              animation.callback,
              fireScrollCallback,
            );
          }
          ...
          if (position !== maxPosition) {
            if (Math.abs(mod) > snapSize / 2) {
              snapPosition = Math.min(
                maxPosition,
                position + (mod > 0 ? snapSize - mod : mod - snapSize),
              );
            } else {
    ...
    onAxisDrag: function (axis, delta) {
      ...
      if (
        (lastDirection !== 0 && dragDirection[axis] !== lastDirection) ||
    ...
    onEvent: function (e) {
      ...
      if (
        (!me.self.isTouching || me.isTouching) && // prevents nested scrolling
        ...
        // is implemented
        (!me.getTranslatable().isScrollParent ||
          (!me.isMouseEvent[browserEvent.type] &&
