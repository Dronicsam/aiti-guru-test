...
Ext.define("Ext.scroll.Indicator", {
  ...
  setValue: function (value) {
    ...
    if (value < 0) {
      length = round(
        max(baseLength + (baseLength * value) / elementSize, minLength),
      );
      position = 0;
    } else if (value > maxScrollPosition) {
      length = round(
        max(
          baseLength - (baseLength * (value - maxScrollPosition)) / elementSize,
          minLength,
        ),
      );
  ...
  privates: {
    ...
    hasOpposite: function () {
      return this.getScroller().isAxisEnabled(
        this.oppositeAxis[this.getAxis()],
      );
    },
    ...
    refreshLength: function () {
      var me = this,
        ...
        sizeAdjust = me.hasOpposite()
          ? baseSizeAdjust + me.size
          : baseSizeAdjust,
        length = Math.max(
          Math.round((elementSize - sizeAdjust) * ratio),
          me.getMinLength(),
        );
