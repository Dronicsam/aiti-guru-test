...
Ext.define("Ext.dom.ElementEvent", {
  ...
  addListener: function (fn, scope, options, caller, manager) {
    ...
    if (
      ...
      Ext.event.publisher.Dom.instance.directEvents[name]
    ) {
      if (options.capture) {
        directCaptures =
          me.directCaptures ||
        ...
        directs =
          me.directs || (me.directs = new Ext.util.Event(me.observable, name));
        added = directs.addListener(fn, scope, options, caller, manager);
      }
    } else if (options.capture) {
      captures =
        me.captures || (me.captures = new Ext.util.Event(me.observable, name));
