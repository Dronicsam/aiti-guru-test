...
Ext.define(
  ...
  function (Element) {
    ...
    return {
      ...
      mixins: ["Ext.util.Positionable", "Ext.mixin.Observable"],
      ...
      inheritableStatics: {
        ...
        normalize: function (prop) {
          return (
            propertyCache[prop] ||
            (propertyCache[prop] = prop.replace(camelRe, camelReplaceFn))
          );
        },
        ...
        _onWindowResize: function () {
          ...
          if (
            now - lastFocusChange < threshold ||
            now - lastKeyboardClose < threshold
          ) {
            ...
            if (
              deltaX === 0 &&
              editableHasFocus &&
              deltaY <= -Element.minKeyboardHeight
            ) {
        ...
        serializeForm: function (form) {
          var fElements =
              form.elements || (DOC.forms[form] || Ext.getDom(form)).elements,
            hasSubmit = false,
      ...
      adjustDirect2DDimension: function (dimension) {
        ...
        dom.style.position = display.match(adjustDirect2DTableRe)
          ? "absolute"
          : "static";
        ...
        floating =
          (parseFloat(currentStyle[dimension]) ||
            parseFloat(currentStyle.msTransformOrigin.split(" ")[originIndex]) *
              2) % 1;
      ...
      getAttribute: function (name, namespace) {
        ...
        return namespace
          ? dom.getAttributeNS(namespace, name) ||
              dom.getAttribute(namespace + ":" + name)
      ...
      getBorderPadding: function () {
        ...
        return {
          beforeX:
            (parseFloat(bordersWidth[borders.l]) || 0) +
            (parseFloat(paddingWidth[paddings.l]) || 0),
          afterX:
            (parseFloat(bordersWidth[borders.r]) || 0) +
            (parseFloat(paddingWidth[paddings.r]) || 0),
          beforeY:
            (parseFloat(bordersWidth[borders.t]) || 0) +
      ...
      getWidth: function (contentWidth, preciseWidth) {
        ...
        if (Ext.supports.BoundingClientRect) {
          ...
          width =
            me.vertical && !Ext.supports.RotatedBoundingClientRect
      ...
      isVisible: function (deep) {
        ...
        for (
          end = dom.ownerDocument.documentElement;
          dom !== end;
          dom = dom.parentNode
        ) {
          ...
          if (
            !dom ||
            dom.nodeType === 11 ||
            visFly.attach(dom).isStyle(VISIBILITY, HIDDEN) ||
      ...
      matchNode: function (dir, start, selector, returnDom) {
        ...
        while (n) {
          if (
            n.nodeType === 1 &&
            (!selector || Ext.fly(n, "_matchNode").is(selector))
          ) {
       ...
       *
       * @param {Ext.dom.Element.VISIBILITY/Ext.dom.Element.DISPLAY/Ext.dom.Element.OFFSETS/Ext.dom.Element.CLIP}
      ...
      translate: (function () {
        var transformStyleName =
          "webkitTransform" in DOC.createElement("div").style
            ? "webkitTransform"
            : "transform";

        return function (x, y, z) {
          this.dom.style[transformStyleName] =
            "translate3d(" +
            (x || 0) +
            "px, " +
            (y || 0) +
      ...
      privates: {
        ...
        doRemoveListener: function (eventName, fn, scope) {
          ...
          if (!me.blockedEvents[eventName]) {
            ...
            translatedEventName = me.eventMap[eventName];
            if (translatedEventName) {
              observableDoRemoveListener.call(
                me,
                translatedEventName,
                fn,
                scope,
  ...
  function (Element) {
    ...
    if (Ext.isWebKit) {
      ...
      eventMap.transitionend =
        Ext.browser.getVendorProperyName("transitionEnd");
      eventMap.animationstart =
        Ext.browser.getVendorProperyName("animationStart");
    ...
    eventOptions.translate =
      ...
      eventOptions.delegate =
      eventOptions.delegated =
      eventOptions.stopEvent =
      eventOptions.preventDefault =
      eventOptions.stopPropagation =
    ...
    Ext.apply(Ext, {
      ...
      removeNode: function (node) {
        ...
        if (el) {
          ...
          node &&
          (node.nodeType === 3 || node.tagName.toUpperCase() !== "BODY")
        ) {
