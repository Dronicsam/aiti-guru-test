...
Ext.define(
  "Ext.event.publisher.Gesture",
  {
    ...
    constructor: function (config) {
      ...
      if (
        !handledDomEvents.length ||
        (supportsTouchEvents && Ext.isWebKit && Ext.os.is.Desktop)
      ) {
    ...
    invokeRecognizers: function (methodName, e) {
      ...
      for (i = 0; i < ln; i++) {
        ...
        if (
          recognizer.isActive &&
          recognizer[methodName].call(recognizer, e) === false
        ) {
    ...
    onTouchMove: function (e) {
      ...
      if (me.isStarted) {
        ...
        if (
          mousePointerType &&
          e.browserEvent.pointerType === mousePointerType &&
          e.buttons === 0
        ) {
