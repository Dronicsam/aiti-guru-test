...
Ext.define("Ext.data.Connection", {
  ...
  isFormUpload: function (options) {
    ...
    if (form) {
      return (
        options.isUpload || this.enctypeRe.test(form.getAttribute("enctype"))
      );
    }
  ...
  setOptions: function (options, scope) {
    ...
    if (options.binaryData) {
      ...
      if (me.nativeBinaryPostSupport()) {
        ...
        if (
          (Ext.isChrome && Ext.chromeVersion < 22) ||
          Ext.isSafari ||
          Ext.isGecko
        ) {
    ...
    urlParams = Ext.isObject(urlParams)
      ? Ext.Object.toQueryString(urlParams)
      : urlParams;
