...
Ext.define("Ext.data.TreeStore", {
  ...
  alias: "store.tree",
  requires: ["Ext.util.Sorter", "Ext.data.TreeModel", "Ext.data.NodeInterface"],
  ...
  applyFields: function (fields, oldFields) {
    ...
    if (fields) {
      if (
        me.defaultRootProperty !== me.self.prototype.config.defaultRootProperty
      ) {
  ...
  onBeforeNodeExpand: function (node, callback, scope, args) {
    ...
    // There are unloaded child nodes in the raw data because of the lazy configuration, load them then call back.
      ...
      reader =
        nodeReader && nodeReader.initialConfig.rootProperty
          ? nodeReader
          : storeReader;
  ...
  beforeNodeRemove: function (parentNode, childNodes) {
    ...
    if (startNode) {
      ...
      me.lastRemoveIndexPlusOne = me.indexOfNextVisibleNode(
  ...
  onNodeSort: function (node, childNodes) {
    ...
    if (
      me.indexOf(node) !== -1 ||
      (node === me.getRoot() && !me.getRootVisible() && node.isExpanded())
    ) {
      Ext.suspendLayouts();
    ...
    if (newRoot) {
      ...
      if (newRoot.fireEventArgs("beforeappend", [null, newRoot]) === false) {
        ...
        // If the passed root was previously the rootNode of another TreeStore, it must be removed from that store
          ...
          (oldOwner = newRoot.getTreeStore()) &&
          oldOwner !== me &&
          newRoot === oldOwner.getRoot()
        ) {
