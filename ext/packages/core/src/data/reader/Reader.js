...
Ext.define(
  "Ext.data.reader.Reader",
  {
    ...
    requires: ["Ext.data.ResultSet", "Ext.XTemplate", "Ext.util.LruCache"],
    ...
    extractData: function (root, readOptions) {
      ...
      for (i = 0; i < length; i++) {
        ...
        if (!record.isModel) {
          ...
          if (
            typeProperty &&
            (nodeType = me.getChildType(schema, node, typeProperty))
          ) {
    ...
    extractRecord: function (
      ...
      var me = this,
        creatorFn =
          (readOptions && readOptions.recordCreator) || me.defaultRecordCreator,
        modelData,
        record;
