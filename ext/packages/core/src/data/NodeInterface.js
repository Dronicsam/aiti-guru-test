      ...
      return {
        ...
        updateInfo: function (commit, info) {
          ...
          for (propName in info) {
            ...
            if (persistentField) {
              ...
              if (
                !settingIndexInNewParent &&
                modified.hasOwnProperty(propName)
              ) {
        ...
        appendChild: function (node, suppressEvents, commit) {
          ...
          if (Ext.isArray(node)) {
            ...
            if (
              suppressEvents !== true &&
              me.fireEventArgs("beforeappend", [me, node]) === false
            ) {
        ...
        removeChild: function (node, erase, suppressEvents, isMove) {
          ...
          if (
            index === -1 ||
            (suppressEvents !== true &&
              me.fireEventArgs("beforeremove", [me, node, !!isMove]) === false)
          ) {
            return false;
          }
          ...
          if (node.nextSibling) {
            ...
            for (
              i = index, childCount = me.childNodes.length;
              i < childCount;
              i++
            ) {
        ...
        insertBefore: function (node, refNode, suppressEvents) {
          ...
          if (
            suppressEvents !== true &&
            me.fireEventArgs("beforeinsert", [me, node, refNode]) === false
          ) {
            return false;
          }
          ...
          for (
            i = refIndex + 1, childCount = me.childNodes.length;
            i < childCount;
            i++
          ) {
            me.childNodes[i].updateInfo(false, {
        ...
        cascade: function () {
          if (Ext.isDefined(Ext.global.console)) {
            Ext.global.console.warn(
              "Ext.data.Node: cascade has been deprecated. Please use cascadeBy instead.",
            );
          }
                ...
                if (me.fireEventArgs("beforeexpand", [me]) !== false) {
                  ...
                  if (recursive) {
                    ...
                    if (
                      ...
                      treeStore = me.getTreeStore();
                      if (
                        treeStore.getProxy().isSynchronous ||
                        me.isBranchLoaded()
                      ) {
                        me.isSynchronousRecursiveExpand = true;
        ...
        collapse: function (recursive, callback, scope) {
          ...
          if (
            !me.isLeaf() &&
            ((!expanded && recursive) ||
              me.fireEventArgs("beforecollapse", [me]) !== false)
          ) {
        ...
        fireEventArgs: function (eventName, args) {
          ...
          if (bubbledEvents[eventName]) {
            for (
              eventSource = this;
              result !== false && eventSource;
              eventSource = (topNode = eventSource).parentNode
            ) {
              if (
                eventSource.hasListeners &&
                eventSource.hasListeners[eventName]
              ) {
