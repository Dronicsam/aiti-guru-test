...
Ext.define("Ext.data.request.Form", {
  ...
  getDoc: function () {
    ...
    return (
      (frame && (frame.contentWindow.document || frame.contentDocument)) ||
  ...
  onComplete: function () {
    ...
    if (me.aborted || me.timedout) {
      ...
      response.responseText =
        ...
        Ext.String.trim(response.statusText) +
        '"}';
      ...
      try {
        ...
        if (doc) {
          ...
          if (doc.body) {
            ...
            if (
              (contentNode = doc.body.firstChild) &&
              /pre/i.test(contentNode.tagName)
            ) {
              response.responseText =
                contentNode.textContent || contentNode.innerText;
            }
