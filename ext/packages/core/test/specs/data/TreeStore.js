describe("Ext.data.TreeStore", function () {
  ...
  describe("getNodeById", function () {
    ...
    it("should find loaded children of collapsed nodes", function () {
      ...
      expect(store.getNodeById("leaf")).toBe(
        store.getRoot().firstChild.firstChild,
      );
    });
    ...
    describe("when loading asynchronously from a url", function () {
      ...
      describe("if the root node is not expanded", function () {
        ...
        describe("if autoLoad is set to true", function () {
          beforeEach(function () {
            spyOn(Ext.data.TreeStore.prototype, "load").andCallFake(
              function () {},
            );
        ...
        beforeEach(function () {
          ...
          store.fillNode(
            ...
            store
              .getProxy()
              .getReader()
              .readRecords(dummyData.children)
              .getRecords(),
          );
        });
      ...
      describe("when the index field is persistent", function () {
        ...
        describe("inserting a single node", function () {
          ...
          it("should add all of its sibling nodes that come after the insertion point to getUpdatedRecords", function () {
            ...
            expect(Ext.Array.contains(records, store.getNodeById(3))).toBe(
              true,
            );
            expect(Ext.Array.contains(records, store.getNodeById(4))).toBe(
              true,
            );
          });
        });
      ...
      describe("moving records", function () {
        describe("within the same parent node", function () {
          beforeEach(function () {
            store
              .getRootNode()
              .insertBefore(store.getNodeById(6), store.getNodeById(1));
          });
        ...
        describe("to a different parent node", function () {
          beforeEach(function () {
            store
              .getNodeById(4)
              .insertBefore(store.getNodeById(1), store.getNodeById(5));
          });
      ...
      describe("moving records when the index field is persistent", function () {
        ...
        describe("within the same parent node", function () {
          ...
          it("should add the node and all sibling nodes after it to getUpdatedRecords", function () {
            ...
            expect(Ext.Array.contains(records, store.getNodeById(1))).toBe(
              true,
            );
            expect(Ext.Array.contains(records, store.getNodeById(2))).toBe(
              true,
            );
            expect(Ext.Array.contains(records, store.getNodeById(6))).toBe(
              true,
            );
          });
        });
        ...
        describe("to a different parent node", function () {
          ...
          it("should add the node, all sibling nodes after it's insertion point, and all siblings after its removal point to getUpdatedRecords", function () {
            ...
            expect(Ext.Array.contains(records, store.getNodeById(5))).toBe(
              true,
            );
            expect(Ext.Array.contains(records, store.getNodeById(6))).toBe(
              true,
    ...
    describe("removing records", function () {
      describe("removing a single record", function () {
        ...
        it("should not add phantom records to the removed collection", function () {
          ...
          expect(Ext.Array.contains(store.getRemovedRecords(), node)).toBe(
            false,
          );
        });
      });
  ...
  describe("Node events bubbled to the root node", function () {
    var spy, root, newNode, removedNode, firstChild, spyArgs;
    ...
    describe("event ordering", function () {
      it("should fire events in the correct order", function () {
        ...
        result = result.join(", ");
        expect(result).toBe(
          "/root/A | /root, /root/A/A.A | /root/A, /root/A/A.B | /root/A, /root/A/A.C | /root/A, /root/A/A.C/A.C.A | /root/A/A.C, /root/A/A.C/A.C.B | /root/A/A.C, /root/A/A.D | /root/A",
        );
        store.destroy();
      });
    });
  ...
  describe("appending to leaf nodes", function () {
    beforeEach(function () {
      ...
      store.fillNode(
        store.getRootNode(),
        store.getProxy().getReader().readRecords(dummyData.children).records,
      );
    });
        ...
        proxy: {
          ...
          reader: {
            typeProperty: function (rawData) {
              ...
              return Ext.String.startsWith(rawData.mtype, "spec.")
                ? rawData.mtype
                : "spec." + rawData.mtype;
            },
          },
    ...
    function doTests(rootVisible) {
      describe("rootVisible = " + rootVisible, function () {
        it("should not include the root node", function () {
          ...
          expect(
            Ext.Array.contains(store.getRejectRecords(), store.getRoot()),
          ).toBe(false);
        });
      });
    }
