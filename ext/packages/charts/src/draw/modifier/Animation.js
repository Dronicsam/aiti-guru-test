...
Ext.define("Ext.draw.modifier.Animation", {
  ...
  applyCustomDuration: function (newDuration, oldDuration) {
    if (newDuration) {
      ...
      //<debug>
      Ext.log.warn(
        "'customDuration' config is deprecated. Use 'customDurations' config instead.",
      );
      //</debug>
    }
  },
    ...
    if (!any) {
      ...
      for (name in changes) {
        ...
        if (
          ...
          if (
            (startValue && startValue.isGradient) ||
            (newValue && newValue.isGradient)
          ) {
            duration = 0;
          }
          ...
          if (duration) {
            ...
            timer.remove =
              changes.removeFromInstance && changes.removeFromInstance[name];
