...
Ext.define(
  "Ext.draw.Matrix",
  {
    ...
    statics: {
       ...
       * @param {Number} y1p
       */
      createAffineMatrixFromTwoPair: function (
        x0,
        y0,
        x1,
        y1,
    ...
    precisionCompensateRect: function (devicePixelRatio, comp) {
      ...
      comp.dx =
        (newDy * y2x - newDx * y2y) / (x2y * yxOnXx - y2y) / devicePixelRatio;
      comp.dy =
        -(newDy * x2x - newDx * x2y) / (x2y * yxOnXx - y2y) / devicePixelRatio;
    },
    ...
    transformBBox: function (bbox, radius, target) {
      ...
      if (radius) {
        ...
        h =
          Math.abs(w0 * xy) + Math.abs(h0 * yy) + Math.abs(scales[1] * radius);
      } else {
