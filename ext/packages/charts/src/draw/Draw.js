...
Ext.define("Ext.draw.Draw", {
  ...
  isBBoxIntersect: function (bbox1, bbox2, padding) {
    padding = padding || 0;
    return (
      Math.max(bbox1.x, bbox2.x) - padding >
  ...
  getAnchors: function (prevX, prevY, curX, curY, nextX, nextY, value) {
    ...
    if (
      (curY > prevY && control1Y < prevY) ||
      (curY < prevY && control1Y > prevY)
    ) {
      control1X +=
        (abs(prevY - control1Y) * (control1X - curX)) / (control1Y - curY);
      control1Y = prevY;
    }
    if (
      (curY > nextY && control2Y < nextY) ||
      (curY < nextY && control2Y > nextY)
    ) {
      control2X -=
        (abs(nextY - control2Y) * (control2X - curX)) / (control2Y - curY);
      control2Y = nextY;
    }
