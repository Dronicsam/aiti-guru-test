  ...
  "Ext.draw.sprite.Sprite",
  {
        ...
        updaters: {
          ...
          bbox: function (attr) {
            var hasRotation = attr.rotationRads !== 0,
              ...
              noRotationCenter =
                attr.rotationCenterX === null || attr.rotationCenterY === null,
              noScalingCenter =
                attr.scalingCenterX === null || attr.scalingCenterY === null;
            ...
            if (
              (hasRotation && noRotationCenter) ||
              (hasScaling && noScalingCenter)
            ) {
    ...
    constructor: function (config) {
      ...
      for (var name in config) {
        if (
          name in processors &&
          me["get" + name.charAt(0).toUpperCase() + name.substr(1)]
        ) {
          Ext.raise(
            "The " +
              me.$className +
    ...
    useAttributes: function (ctx, rect) {
      ...
      for (id in canvasAttributes) {
        if (
          canvasAttributes[id] !== undefined &&
          canvasAttributes[id] !== ctx[id]
        ) {
