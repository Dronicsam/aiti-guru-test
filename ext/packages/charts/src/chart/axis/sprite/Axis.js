...
Ext.define("Ext.chart.axis.sprite.Axis", {
    ...
    if (majorTicks) {
      switch (docked) {
        ...
        case "gauge":
          ...
          me.iterate(majorTicks, function (position, labelText, i) {
            position =
              ((position - attr.min) / (attr.max - attr.min + 1)) *
                attr.totalAngle -
              attr.totalAngle +
              gaugeAngles.start;
            ctx.moveTo(
  ...
  renderLabels: function (surface, ctx, layoutContext, clipRect) {
    var me = this,
      ...
      tickPadding =
        Math.max(attr.majorTickSize, attr.minorTickSize) + attr.lineWidth,
    ...
    if (majorTicks && label && !label.attr.hidden) {
      ...
      switch (docked) {
        case "left":
          ...
          switch (label.attr.textAlign) {
            case "start":
              translation =
                surface.roundPixel(titlePadding + dx) - halfLineWidth;
              break;
            case "end":
              translation =
                surface.roundPixel(clipRect[2] - tickPadding + dx) -
                halfLineWidth;
              break;
            default: // 'center'
              translation =
        ...
        case "right":
          ...
          switch (label.attr.textAlign) {
            case "start":
              translation =
                surface.roundPixel(tickPadding + dx) + halfLineWidth;
              break;
            case "end":
              translation =
                surface.roundPixel(clipRect[2] - titlePadding + dx) +
                halfLineWidth;
              break;
            default: // 'center'
              translation =
      ...
      if (docked === "left" || docked === "right") {
            ...
            label.setAttributes(
              {
                ...
                translationX:
                  ...
                  (surface.roundPixel(position) / attr.max) *
                    attr.length *
                    Math.cos(attr.baseRotation + Math.PI / 2),
                translationY:
                  attr.centerY -
                  (surface.roundPixel(position) / attr.max) *
                    attr.length *
        ...
        labelOffset =
          attr.majorTickSize +
          attr.lineWidth * 0.5 +
          (parseInt(label.attr.fontSize, 10) || 10) / 2;
        me.iterate(majorTicks, function (position, labelText, i) {
          if (labelText === undefined) {
            return;
          }
            ...
            label.setAttributes(
              {
                text: String(text),
                translationX:
                  attr.centerX + (attr.length + 10) * Math.cos(angle),
                translationY:
                  attr.centerY + (attr.length + 10) * Math.sin(angle),
              },
              true,
              true,
            );
  ...
  renderLimits: function (clipRect) {
    ...
    if (position === "left" || position === "right") {
      for (i = 0, ln = limits.length; i < ln; i++) {
        ...
        value = Ext.isString(limit.value)
          ? axis.getCoordFor(limit.value)
          : limit.value;
      ...
      for (i = 0, ln = limits.length; i < ln; i++) {
        ...
        if (limit.line.title) {
          ...
          titleBBox = titles.getBBoxFor(titles.position - 1);
          titlePosition =
            limit.line.title.position || (position === "top" ? "end" : "start");
          switch (titlePosition) {
            case "start":
      ...
      for (i = 0, ln = limits.length; i < ln; i++) {
        ...
        if (limit.line.title) {
          ...
          titleBBox = titles.getBBoxFor(titles.position - 1);
          titleFlip =
            (value > -0.5 * Math.PI && value < 0.5 * Math.PI) ||
          ...
          titles.setAttributesFor(titles.position - 1, {
            x:
              attr.centerX +
              0.5 * attr.length * Math.cos(value) +
