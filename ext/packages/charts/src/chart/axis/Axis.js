...
Ext.define("Ext.chart.axis.Axis", {
  ...
  applyFloating: function (floating, oldFloating) {
    ...
    if (Ext.isObject(floating)) {
      if (oldFloating && oldFloating.alongAxis) {
        delete this.getChart().getAxis(oldFloating.alongAxis).floatingAxes[
          this.getId()
        ];
      }
      return floating;
  ...
  applyVisibleRange: function (visibleRange, oldVisibleRange) {
    ...
    if (
      oldVisibleRange &&
      visibleRange[0] === oldVisibleRange[0] &&
  ...
  resolveListenerScope: function (defaultScope) {
    ...
    if (!namedScope) {
      ...
      if (
        scope === chart &&
        !chart.getInheritedConfig("defaultListenerScope")
      ) {
        scope = me;
      }
    }
