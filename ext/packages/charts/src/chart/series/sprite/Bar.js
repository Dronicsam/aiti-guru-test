...
Ext.define("Ext.chart.series.sprite.Bar", {
  ...
  drawLabel: function (text, dataX, dataStartY, dataY, labelId) {
    ...
    if (
      (labelOrientation === "horizontal" && attr.flipXY) ||
      (labelOrientation === "vertical" && !attr.flipXY) ||
      !labelOrientation
    ) {
      labelY =
        labelDisplay === "insideStart"
          ? dataStartY + halfWidth
          : dataY - halfWidth;
    } else {
      labelY =
        labelDisplay === "insideStart"
    ...
    if (
      Math.abs(dataY - dataStartY) <= halfWidth * 2 ||
  ...
  renderClipped: function (surface, ctx, clip, rect) {
    ...
    var me = this,
      ...
      minBarWidth =
        (Math.min(maxBarWidth, attr.maxBarWidth) -
          inGroupGapWidth * (groupCount - 1)) /
        groupCount,
