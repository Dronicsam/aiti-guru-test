...
Ext.define("Ext.chart.series.sprite.PieSlice", {
  ...
  setGradientBBox: function (ctx, rect) {
    var me = this,
      attr = me.attr,
      hasGradients =
        (attr.fillStyle && attr.fillStyle.isGradient) ||
  ...
  render: function (surface, ctx, clip, rect) {
    ...
    if (attr.renderer) {
      ...
      changes = Ext.callback(
        attr.renderer,
        null,
        [me, itemCfg, me.rendererData, me.rendererIndex],
        0,
        me.getSeries(),
      );
  ...
  placeLabel: function () {
    ...
    if (!attr.rotateLabels) {
      ...
      //<debug>
      Ext.log.warn(
        "'series.style.rotateLabels' config is deprecated. " +
    ...
    labelCfg.calloutColor =
      (calloutLine && calloutLine.color) || me.attr.fillStyle;
    if (calloutLine) {
