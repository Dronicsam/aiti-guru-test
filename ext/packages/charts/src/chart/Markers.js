...
Ext.define("Ext.chart.Markers", {
  ...
  putMarkerFor: function (
    ...
    var me = this,
      categoryInstances =
        me.categories[category] || (me.categories[category] = {}),
      instance;
    ...
    if (instance) {
      ...
      if (!keepRevision) {
        instance.revision =
          me.revisions[category] || (me.revisions[category] = 1);
      }
    }
  },
  ...
  render: function (surface, ctx, clipRect) {
    ...
    for (i = 0, ln = me.instances.length; i < ln; i++) {
      ...
      if (
        instance.hidden ||
        instance.revision !== revisions[instance.category]
      ) {
        continue;
      }
      ctx.save();
