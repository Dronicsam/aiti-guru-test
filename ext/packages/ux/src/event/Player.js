...
Ext.define("Ext.ux.event.Player", function (Player) {
  ...
  Ext.each(
    ["blur", "change", "focus", "resize", "scroll", "select"],
    function (type) {
  ...
  return {
    ...
    processEvents: function () {
      ...
      while ((eventDescriptor = me.peekEvent()) !== null) {
        ...
        if (eventDescriptor.keyframe) {
          ...
          eventDescriptor.ts <= me.getTimeIndex() &&
    ...
    injectEvent: function (target, event) {
      ...
      if (type === "mduclick") {
        return (
          me.injectEvent(target, Ext.applyIf({ type: "mousedown" }, event)) &&
