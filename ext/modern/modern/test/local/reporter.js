...
jasmine.PrettyPrinter.prototype.format = function (value) {
  ...
  try {
    if (value === jasmine.undefined) {
      ...
      //override of value instanceof jasmine.Matchers.Any
      this.emitScalar(value.toString());
    } else if (typeof value === "string") {
      this.emitString(value);
    } else if (jasmine.isSpy(value)) {
...
jasmine.browser.isIE7 =
  jasmine.browser.isIE && !!window.XMLHttpRequest && !document.documentMode;
jasmine.browser.isIE8 =
  jasmine.browser.isIE &&
  !!window.XMLHttpRequest &&
  !!document.documentMode &&
  !window.performance;
jasmine.browser.isIE9 = jasmine.browser.isIE && !!window.performance;
jasmine.browser.isSafari3 =
  /safari/.test(navigator.userAgent.toLowerCase()) &&
  /version\/3/.test(navigator.userAgent.toLowerCase());
jasmine.browser.isOpera = !!window.opera;
jasmine.browser.isOpera11 =
  jasmine.browser.isOpera && parseInt(window.opera.version(), 10) > 10;
jasmine.array = {};
...
Test.SandBoxImpl.prototype.createIframe = function () {
  ...
  var src = me.options.quirksMode
    ? "iframe-quirks.html?loadSpecs=true"
    : "iframe.html?loadSpecs=true";
...
Test.SandBoxImpl.prototype.save = function (spec) {
  ...
  if (length != lwas) {
    if (!window.headless) {
      this.reporter.log(
        ">> Warning the document.body dom element contains childNodes after spec execution !<br/>" +
          "Spec : " +
          jasmine.util.htmlEscape(spec.getFullName()) +
          ' <a href="?' +
      ...
      this.reporter.log(
        "Warning: " +
          spec.getFullName() +
          "doesn't clean properly the document.body.",
      );
    }
 ...
 */
Test.CodeHighLighter.prototype.matchObjects = function (
  value,
  index,
  matchedObjects,
  css,
  ...
  matchedObjects.push({
    ...
    value:
      ...
      css +
      '">' +
      jasmine.util.htmlEscape(value).replace("$", "$\b") +
      "</span>",
    start: index,
 ...
 * @return {Boolean} Returns <tt>true</tt> is the match is inside another.
 */
Test.CodeHighLighter.prototype.isInside = function (
  matchedObject,
  matchedOthers,
) {
 ...
 */
Test.CodeHighLighter.prototype.fixOverlaps = function (
  matchedObjects,
  matchedOthers,
  protect,
) {
...
Test.CodeHighLighter.prototype.saveStringsAndComments = function () {
  ...
  this.matchedComments = this.fixOverlaps(
    this.matchedComments,
    this.matchedStrings,
    "%%%%comment%%%%",
  );
  this.matchedStrings = this.fixOverlaps(
    this.matchedStrings,
    this.matchedComments,
    "%%%%string%%%%",
  );
...
Test.CodeHighLighter.prototype.processOperatorsNumbersKeywords = function () {
  ...
  this.source = jasmine.util
    .htmlEscape(this.source)
    .replace(regexps.operators, '<span class="jsHlOperator">$1</span>')
    .replace(regexps.numbers, '<span class="jsHlNumber">$1</span>');

  for (i = 0; i < length; i++) {
    this.source = this.source.replace(
      keywords[i],
      '<span class="jsHlKeyword">$1</span>',
    );
  }
};
...
Test.CodeHighLighter.prototype.renderJsSources = function () {
  ...
  if (source) {
    ...
    result =
      '<table border="0" cellpadding="0" cellspacing="0"><tbody><tr><td class="lineNumbers">';
    for (i = 0; i < length; i++) {
      errorCls = "";
      lineNumberCls = "";
      if (lineNumber) {
      ...
      if (linesFromJsCoverage) {
        lineNumberCls = !isNaN(linesFromJsCoverage[i + 1])
          ? " lineNumberGreen"
          : "";
        lineNumberCls =
          linesFromJsCoverage[i + 1] === 0 ? " lineNumberRed" : lineNumberCls;
      }
      result +=
...
Test.CodeHighLighter.prototype.prepareIndent = function () {
  ...
  this.source = this.source
    .replace(regexps.multipleWhiteSpaces, " ")
    .replace(regexps.semiColon, ";\n")
  ...
  this.source = this.source
    .replace(regexps.arrayWithOneElement, "[$1]")
    .replace(regexps.emptyObject, "{}")
    .replace(regexps.commaBracket, ", {")
    .replace(regexps.unwantedNewLines, "$1");
};
...
Test.BadGlobalsImpl.prototype.report = function (info, suite) {
  ...
  for (property in win) {
    if (!allowedGlobals[property]) {
      ...
      message =
        ">> Bad global variable found in " +
        (suite ? suite.description : "global scope") +
        "<br/>" +
        property +
        " = " +
        value;
  ...
  updateTotal: function () {
    ...
    for (filename in coverage) {
      ...
      if (create) {
        ...
        this.coverage[filename].percentage = (
          (fexecuted / fstatements) *
          100
        ).toFixed(2);
      }
    }
  ...
  for (i = 0; i < length; i++) {
    ...
    if (block.func) {
      children.push({
        ...
        html:
          block.func.typeName ||
          'it("' + jasmine.util.htmlEscape(config.spec.description) + '")',
      });
...
Test.panel.jsCoverageSummary.prototype.renderSummary = function () {
  ...
  if (!this.summary) {
    result =
      '<table class="summary" border="0" cellpadding="0" cellspacing="0"><tbody>';
    result +=
      '<tr class="line header"><td class="fileName">File</td><td class="statements">Statements</td><td class="executed">Executed</td><td class="percentage">Percentage</td></tr>';
    ...
    result +=
      '<td class="percentage">' +
      this.renderPercentage(Test.jsCoverage.percentage) +
      "</td>";
    result += "</tr>";
    ...
    for (filename in coverage) {
      ...
      result +=
        '<td class="statements">' + coverage[filename].statements + "</td>";
      result += '<td class="executed">' + coverage[filename].executed + "</td>";
      result +=
        '<td class="percentage">' +
        this.renderPercentage(coverage[filename].percentage) +
        "</td>";
      result += "</tr>";
    }
    result += "</tbody></table>";
 ...
 */
Test.panel.StackTrace.prototype.extractRe =
  /((http:\/\/|file:\/\/\/).*\.js)[^:]*:(\d*)/;
 ...
 */
Test.panel.StackTrace.prototype.jscoverageFileRe =
  /(http:\/\/|file:\/\/\/)[^\/]*/;

Test.panel.StackTrace.prototype.renderTraceFileSource = function (
  file,
  lineNumber,
) {
  ...
  if (Test.SandBox.getWin()._$jscoverage) {
    instrumented_file =
      SandBox.getWin()._$jscoverage[file.replace(this.jscoverageFileRe, "")];
...
Test.panel.TabPanel.prototype.addTab = function (cls, name, persist) {
  var el = this.toolbar.appendChild(
    new jasmine.Dom({
      tag: "span",
      cls: "toolbarTab " + cls,
      html: name,
...
Test.panel.TreeGrid = function (config) {
  ...
  me.el = document.body.appendChild(
    new jasmine.Dom({
      tag: "div",
      cls: "treegrid",
...
Test.panel.TreeGrid.prototype.addSuite = function (suite) {
  ...
  this.suitesEls[suite.id] = new jasmine.Dom({
    ...
    cls:
      "suite" +
      (suite.isDisabled() ? " disabled" : "") +
      (padding === 18 ? " parent-suite" : ""),
    style: {
...
Test.panel.TreeGrid.prototype.onMouseMove = function (event) {
  ...
  if (this.pageY) {
    ...
    diff = Math.max(
      200,
      this.startHeight - ((event.pageY || event.clientY) - this.pageY),
    );
...
Test.panel.TreeGrid.prototype.onExpanderClick = function (resultsEl, e) {
  var el = e.target || e.srcElement,
    ...
    isCollapsed = jasmine.Dom.hasCls(el, expanderCollapseCls)
      ? "removeCls"
      : "addCls";
  ...
  var children = [
    {
      ...
      html:
        "Actual: " +
        jasmine.pp(result.actual) +
        "\nExpected: " +
        jasmine.pp(result.expected) +
        "\nMatcher: " +
        result.matcherName +
        ".",
    },
...
Test.Reporter.prototype.reportRunnerStarting = function (runner) {
  ...
  if (!this.options.remote) {
    this.log(
      ">> Warning! Because you access TestReporter locally, stack trace report isn't available.",
      "warning",
    );
  }
...
Test.Reporter.prototype.reportSuiteResults = function (suite) {
  ...
  if (suite.isEnabled()) {
    if (this.options.showTimings) {
      suite.time = (
        (new Date().getTime() - suite.startedAt.getTime()) /
        1000
      ).toFixed(3);
    }
    ...
    if (suiteEl) {
      ...
      if (Test.jsCoverage.isEnabled()) {
        this.treeGrid.updateSuiteEl(
          suite,
          Test.jsCoverage.getSuiteCoverage(suite),
        );
      }
...
Test.Reporter.prototype.reportSpecStarting = function (spec) {
  ...
  if (spec.isEnabled()) {
    ...
    this.treeGrid.setStatus(
      "Running: " + jasmine.util.htmlEscape(spec.getFullName()),
    );
  }
};
...
Test.Reporter.prototype.reportSpecResults = function (spec) {
  ...
  if (spec.isEnabled()) {
    if (this.options.showTimings) {
      spec.time = (
        (new Date().getTime() - spec.startedAt.getTime()) /
        1000
      ).toFixed(3);
    }
    ...
    if (
      (status === "failed" || this.options.showPassed) &&
      spec.isEnabled() &&
      this.treeGrid
    ) {
...
Test.Reporter.prototype.renderResults = function (runner) {
  ...
  message =
    this.runnedSpecsCount +
    " spec" +
    (this.runnedSpecsCount === 1 ? "" : "s") +
    " ran, " +
    this.failedSpecsCount +
    " failure" +
    (this.failedSpecsCount === 1 ? "" : "s") +
    " and " +
    this.disabledSpecsCount +
    " disabled";
