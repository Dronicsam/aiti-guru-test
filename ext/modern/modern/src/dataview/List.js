...
Ext.define("Ext.dataview.List", {
  ...
  handleItemUpdates: function (y) {
    ...
    me.topVisibleIndex = topVisibleIndex = Math.max(
      0,
      itemMap.findIndex(-y) || 0,
    );
    ...
    if (currentTopVisibleIndex !== topVisibleIndex) {
      ...
      if (currentTopVisibleIndex > topVisibleIndex) {
        ...
        if (bufferDistance < minimumBufferDistance) {
          updateCount = Math.min(
            itemsCount,
            minimumBufferDistance - bufferDistance,
          );

          if (updateCount == itemsCount) {
            me.topRenderedIndex = topRenderedIndex = Math.max(
              0,
              topVisibleIndex - (bufferSize - minimumBufferDistance),
            );
            // Update all
      ...
      // When we are scrolling down
        ...
        if (bufferDistance < minimumBufferDistance) {
          ...
          if (updateCount == itemsCount) {
            me.topRenderedIndex = topRenderedIndex = Math.min(
              lastIndex - itemsCount,
              topVisibleIndex - minimumBufferDistance,
            );
            // Update all
            for (i = 0; i < updateCount; i++) {
              itemIndex = topRenderedIndex + i;
  ...
  handleItemHeights: function () {
    ...
    for (i = 0; i < ln; i++) {
      ...
      if (itemIndex !== null) {
        if (variableHeights) {
          height = useSimpleItems
            ? item.element.getHeight()
            : item.element.getFirstChild().getHeight();
  ...
  handlePinnedHeader: function (y) {
    ...
    if (record) {
      ...
      if (
        y >= 0 ||
        (closestHeader === 0 && totalScrollDockTopHeight + y >= 0) ||
        (closestHeader === 0 && -y <= headerMap[closestHeader])
      ) {
        transY = -10000;
      }
  ...
  updatePinHeaders: function (pinnedHeaders) {
    if (this.isPainted()) {
      this.pinnedHeader.translate(
        0,
        pinnedHeaders ? this.pinnedHeader.$position : -10000,
      );
    }
  },
  ...
  parseEvent: function (e) {
    var me = this,
      target = Ext.fly(e.currentTarget).findParent(
  ...
  doDisclose: function (me, record, item, index, e) {
    ...
    if (onItemDisclosure && onItemDisclosure.handler) {
      onItemDisclosure.handler.call(
        onItemDisclosure.scope || me,
        record,
        item,
        index,
        e,
  ...
  onItemTrigger: function (me, index, target, record, e) {
    if (
      !(
        ...
        Ext.fly(e.target).hasCls(this.getBaseCls() + "-disclosure")
      )
    ) {
      this.callParent(arguments);
    }
