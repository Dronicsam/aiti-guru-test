...
Ext.define("Ext.Panel", {
  ...
  alignTo: function (component, alignment) {
    ...
    var LineSegment = Ext.util.LineSegment,
      alignToElement = component.isComponent
      ...
      centerLineSegment = new LineSegment(
        boxCenterPoint,
    ...
    if (
      centerLineSegment.intersects(new LineSegment(leftTopPoint, rightTopPoint))
    ) {
      tipX = Math.min(
        Math.max(alignToCenterX, left + tipWidth),
        right - tipWidth,
      );
      tipY = top;
      ...
      centerLineSegment.intersects(
        new LineSegment(leftTopPoint, leftBottomPoint),
      )
    ) {
      tipX = left;
      tipY = Math.min(
        Math.max(alignToCenterY + tipWidth / 2, tipWidth * 1.6),
        bottom - tipWidth / 2.2,
      );
      ...
      centerLineSegment.intersects(
        new LineSegment(leftBottomPoint, rightBottomPoint),
      )
    ) {
      tipX = Math.min(
        Math.max(alignToCenterX, left + tipWidth),
      ...
      centerLineSegment.intersects(
        new LineSegment(rightTopPoint, rightBottomPoint),
      )
    ) {
      tipX = right;
      tipY = Math.max(
        Math.min(alignToCenterY - tipHeight, bottom - tipWidth * 1.3),
        tipWidth / 2,
      );
      offsetLeft = -tipHeight - 10;
      tipPosition = "right";
