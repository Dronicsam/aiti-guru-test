...
Ext.define("Ext.layout.FlexBox", {
  ...
  updateOrient: function (orient, oldOrient) {
    ...
    if (oldOrient) {
      container.un(
        oldOrient === "horizontal" ? "widthchange" : "heightchange",
        "onItemSizeChange",
        this,
        delegation,
      );
    ...
    container.on(
      orient === "horizontal" ? "widthchange" : "heightchange",
      "onItemSizeChange",
      this,
      delegation,
    );
  ...
  refreshItemSizeState: function (item) {
    ...
    if (isInner) {
      ...
      if (this.getAlign() === "stretch") {
        layoutSizeFlags |=
          containerSizeFlags &
          (dimension === "width" ? LAYOUT_HEIGHT : LAYOUT_WIDTH);
      }

      if (item.getFlex()) {
        layoutSizeFlags |=
          containerSizeFlags &
          (dimension === "width" ? LAYOUT_WIDTH : LAYOUT_HEIGHT);
      }
    }
