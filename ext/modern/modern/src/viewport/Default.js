...
Ext.define("Ext.viewport.Default", {
    ...
    if (!Ext.os.is.Android || Ext.browser.is.ChromeMobile) {
      if (me.supportsOrientation()) {
        me.addWindowListener(
          "orientationchange",
          bind(me.onOrientationChange, me),
        );
      } else {
  ...
  doBlurInput: function (e) {
    ...
    if (
      focusedElement &&
      focusedElement.blur &&
      focusedElement.nodeName.toUpperCase() != "BODY" &&
  ...
  doPreventPanning: function (e) {
    ...
    // the screen.
    if (
      this.isInteractiveWebComponentRegEx.test(target.tagName) &&
      e.touches &&
      e.touches.length > 0
    ) {
      touch = e.touches[0];
      if (
        touch &&
        touch.target &&
        this.isInputRegex.test(touch.target.tagName)
      ) {
        return;
      }
    }
  ...
  fireOrientationChangeEvent: function (newOrientation, oldOrientation) {
    ...
    Ext.getBody().replaceCls(
      clsPrefix + oldOrientation,
      clsPrefix + newOrientation,
    );
  ...
  showMenu: function (side) {
    ...
    size =
      side == "left" || side == "right"
        ? menu.element.getWidth()
        : menu.element.getHeight();
