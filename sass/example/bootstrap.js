  ...
  (function (emptyFn) {
    var doc = document,
      ...
      Boot = {
        ...
        detectPlatformTags: function () {
          ...
          _tags.androidstock =
            _tags.android <= 4.3 && (_tags.safari || _tags.silk);
        ...
        getHead: function () {
          return (
            Boot.docHead ||
            (Boot.docHead = doc.head || doc.getElementsByTagName("head")[0])
          );
        },
      ...
      notify: function () {
        ...
        if (!me.done) {
          var error = me.error,
            ...
            delay =
              "delay" in me ? me.delay : error ? 1 : Boot.config.chainDelay,
            scope = me.scope || me;
          me.done = true;
          if (fn) {
            if (delay === 0 || delay > 0) {
    ...
    function Entry(cfg) {
      ...
      if (Boot.config.disableCaching) {
        ...
        if (buster) {
          busterParam =
            (loader && loader.cacheParam) || Boot.config.disableCachingParam;
  ...
  (function () {
    ...
    Asset.prototype = {
      shouldCache: function () {
        return (
          _storage &&
          this.assetConfig.update &&
          this.assetConfig.hash &&
      ...
      is: function (asset) {
        return (
          ...
          this.assetConfig &&
          asset.assetConfig &&
          this.assetConfig.hash === asset.assetConfig.hash
        );
      },
    ...
    var Manifest = function (cfg) {
      ...
      this.loadOrderMap = this.loadOrder
        ? Boot.createLoadOrderMap(this.loadOrder)
        : null;
    ...
    var _listeners = [],
      ...
      Microloader = {
        init: function () {
          ...
          Microloader.appId = microloaderElement
            ? microloaderElement.getAttribute("data-app")
            : "";
        ...
        run: function () {
          ...
          if (typeof manifest === "string") {
            var extension = ".json",
              url =
                manifest.indexOf(extension) ===
                manifest.length - extension.length
                  ? manifest
                  : manifest + ".json",
            ...
            if (!Microloader.manifest.is(manifest)) {
              ...
              for (prop in newAssets) {
                ...
                if (
                  include &&
                  (!currentAsset ||
                    (newAsset.shouldCache() && !currentAsset.is(newAsset)))
                ) {
                  _debug(
              ...
              for (prop in currentAssets) {
                ...
                if (
                  !include ||
                  !newAsset ||
                  (currentAsset.shouldCache() && !newAsset.shouldCache())
                ) {
                  _debug(
                    "Asset: " +
